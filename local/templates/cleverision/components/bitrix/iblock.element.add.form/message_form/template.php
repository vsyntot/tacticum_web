...
if (!empty($arResult["ERRORS"])):?>
    ...
    <script>
        if (window.main) {
            window.main.notificationWarning('Не удалось отправить сообщение', '<?=htmlspecialchars($arResult["ERRORS"][0], ENT_QUOTES)?>');
        }
    </script>
<?endif;
if (strlen($arResult["MESSAGE"]) > 0):?>
    ...
    <script>
        if (window.main) {
            window.main.notificationSuccess('', 'Сообщение отправлено');
        }
    </script>
<?endif?>
<div class="cmpSendMessage">
    <div class="container">
        <div class="title">
            Отправка сообщения
        </div>
        <div class="description">Вы можете отправить заявку на какую-либо услугу, или задать вопрос. Мы ответим в ближайшее время (как правило, в течении одного рабочего дня). Укажите ваше имя и контактные данные для того, чтобы мы могли ответить на ваш запрос. Мы не передаем данные третьим лицам и не присылаем навязчивую рекламу.</div>
        <form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
            <?=bitrix_sessid_post()?>
            <?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>

            <div class="row form align-items-stretch">
                    ...
                    <div class="d-flex flex-column h-100">
                        <?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
                            <?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
                                ...
                                ?>
                                <div class="label">Текст сообщения<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?>:</div>
                                <?
                    ...
                    <div class="d-flex flex-column">
                        <?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
                            ...
                            foreach ($arResult["PROPERTY_LIST"] as $propertyID):
                                ...
                                if ($propertyID === "NAME") {
                                    ?>
                                    <input type="hidden" name="PROPERTY[<?=$propertyID?>][0]" size="25" value="Сообщение <?=time()?>"  />
                                    <?
                                    continue;
                                }
                                ?>
                                <div class="label"><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?>:</div>
                                <?
                                if (intval($propertyID) > 0)
                                {
                                    if (
                        ...
                        <div class="submitWrapper">
                            <input type="submit" name="iblock_submit" value="ОТПРАВИТЬ СООБЩЕНИЕ">
                        </div>
                    </div>
                </div>
            </div>
