...
if (!empty($arResult["ERRORS"])):?>
    ...
    <script>
        if (window.main) {
            window.main.notificationWarning('Не удалось отправить заявку', '<?=htmlspecialchars($arResult["ERRORS"][0], ENT_QUOTES)?>');
        }
    </script>
<?endif;
if (strlen($arResult["MESSAGE"]) > 0):?>
    ...
    <script>
        if (window.main) {
            window.main.notificationSuccess('Заявка отправлена');
            if ($.fn.modal) {
        ...
        <form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
            ...
            <div class="modal-header">
                <div class="container-fluid">
                    <div class="title">
                        Заявка на выполнение услуг
                    </div>
                    <div class="desc">
                        Укажите номер телефона, имя и удобное время для звонка — мы свяжемся с вами для уточнения параметров выбранных услуг
                    </div>
                    <div class="serviceCounter">
                        Всего выбрано услуг: <span class="counter">0</span>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
                            ...
                            foreach ($arResult["PROPERTY_LIST"] as $propertyID):
                                ...
                                if ($propertyID === "NAME") {
                                    ?>
                                    <input type="hidden" name="PROPERTY[<?=$propertyID?>][0]" value="Сообщение <?=time()?>" />
                                    <?
                                    continue;
                                }
                                if ($propertyID === "PREVIEW_TEXT") {
                                    ?>
                                    <input type="hidden" class="selectedServicesInput" name="PROPERTY[<?=$propertyID?>][0]" value=" " />
                                    <?
                                    continue;
                                }
                                ?>
            ...
            <div class="modal-footer">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12 col-md desc">
                            Отправляя данные вы подтверждаете согласие с хранением и обработкой персональных данных. Мы не передаем информацию третьим лицам. Без спама.
                        </div>
