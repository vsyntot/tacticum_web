...
| Cluster | Gap IDs | Current Risk |
|---|---|---|
| Domain iblock placement | `CSG-001` - `CSG-006` | Product FAQ/services/clients/relation/proof-audit foundation, FAQ section sync, FAQ fallback retirement and `/agents/` vs `/aiagents/` boundary guard are in place; proof public rendering remains owner-gated |
| Page section content | `CSG-007`, `CSG-008`, `CSG-012` | Wave 1 page-level sections now render from `page_sections/page_blocks` on production; narrow iblocks remain protected; approved wave 1 fallback bodies are retired; wave 2 is shadow-seeded and has local runtime guards, but remains PHP fallback until owner live/source approval |
| Guards / release evidence | `CSG-009` - `CSG-011` | Audit/governance/runbook exists; FAQ/services/proof/aiagents production evidence passed; page-content source HTTP/audit/SEO and targeted Chrome-capable visual/browser evidence passed after switch |
| Page-section model | `CSG-007`, `CSG-008`, `CSG-012` | Draft and approved `page_sections/page_blocks` models, checker, migration CLI, seed/live tooling, renderer foundation, audit scope and fallback-retirement checker exist; production source switch and approved fallback retirement are active/rechecked for wave 1; wave 2 shadow seed passed and local runtime integration is ready for deploy |
...
- Production wave 2 page-content shadow seed passed on 06.06.2026 for `/`, `/about/`, `/calculator/` and `/aiagents/`: 11 new sections, 43 new blocks, strict audit with 20 active sections, 80 active blocks and `orphan_blocks=0`; scoped fallback HTTP check passed with zero Bitrix markers while wave 1 remained Bitrix/live.
- Page-content live-status promotion remains owner-gated by `content-storage-page-content-live-approval-template.php`, `content-storage-page-content-live-approval-check.mjs` and `content-storage-page-content-live-apply.php`; these tools approve/apply only `page_sections.MIGRATION_STATUS`. The source switch is a separate environment/config action. Fallback retirement is guarded by `content-storage-page-content-fallback-retirement-template.php` and `content-storage-page-content-fallback-retirement-check.mjs`, including scoped wave 2 approval files; production wave 1 approval passed with production evidence `9/9` and owner gates `5/5`. The checker itself does not remove files; code retirement removed only the approved wave 1 fallback bodies, deploy rechecks passed, and governance guards against static fallback reintroduction.
...
- Owner-approved proof tagging apply passed on production: `platform proof_items_total=6`, `agents proof_items_total=6`, `dev proof_items_total=6`, `forum proof_items_total=5`; public product proof rendering remains blocked until owners approve `public_render_approved=true` and `PUBLIC_RENDER_APPROVED=Y` is applied.
- Proof public rendering now has implementation-ready runtime/tooling: `content-storage-proof-public-render-apply.php` creates/checks the durable `PUBLIC_RENDER_APPROVED` gate, verifies current PRODUCT tags, changes only that flag, product runtime reads `cases/feedback/clients` only through `PROPERTY_PRODUCT + PUBLIC_RENDER_APPROVED=Y`, and product source HTTP evidence can require `proof_source=iblock` or `readiness`.
...
- `content-storage-page-content-migration.php` dry-runs the `page_sections/page_blocks` schema and rejects `--apply` until an approved owner-gated model is supplied; `content-storage-page-content-seed.php` seeds/updates wave 1 and wave 2 rows in shadow mode; `content-storage-page-content-live-approval-check.mjs` validates no-raw-copy live-status approvals; `content-storage-page-content-live-apply.php` changes only `MIGRATION_STATUS`; live runtime foundation reads Bitrix only when `page_content.source=bitrix` and section status is `live`; `content-storage-audit.php --scope=page-content --strict --json` validates schema readiness and aggregate row evidence after apply.
