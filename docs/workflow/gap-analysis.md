...
Production is stable after the public E2E hygiene deploy. Public pages return 200, Chrome-free HTTP/SEO/content checks pass, rendered content hygiene is clean, SEO guards pass, robots/sitemaps are reachable, `/offer/sitemap.php` has 1118 live 200 URLs, and page/product content sources are Bitrix. The immediate release-hygiene gaps for missing `lang=ru`, stale generated Tailwind artifact, stale static sitemap `lastmod` and stale sitemap index lastmods are closed. `robots.txt` was rechecked and left unchanged because it already points to the HTTPS sitemap. Webmanifest MIME hygiene is implemented locally through `.htaccess` plus `seo-check` guard and remains pending production MIME evidence. Remaining product/content gaps are mostly owner-gated: heavy English/internal terminology, `/price/` public label governance, `/offer/` synthetic example disclosure, supporting-page polish and accepted monitoring for `/aiagents/` vs `/agents/` plus CSP report-only. Production-server Chrome-based smoke is still blocked by missing Chrome/Chromium on that host, so browser evidence must run from a Chrome-capable local/CI runner or after configuring `CHROME_PATH`.

### Current Gap Coverage

| Cluster | Gap IDs | Current Risk |
|---|---|---|
| Technical release hygiene | `PUBLIC-E2E-001`, `PUBLIC-E2E-002`, `PUBLIC-E2E-003`, `PUBLIC-E2E-014` | Closed for HTTP/SEO/content scope: `lang` rendering and guards are deployed, production `release:public-precheck:prod` confirmed `lang=ru` for 13/13 pages, Tailwind artifact is regenerated, static sitemap artifact is regenerated and production `seo:check:prod` passed. `PUBLIC-E2E-002` keeps only a server-Chrome caveat because the production host lacks Chrome/Chromium for visual/browser automation. |
| SEO infrastructure | `PUBLIC-E2E-004`, `PUBLIC-E2E-010` | `PUBLIC-E2E-004` closed by sitemap index lastmod update and production `seo:check:prod`; `PUBLIC-E2E-010` is implemented locally with `.webmanifest` MIME mapping and production HTTP guard. It closes after deploy if `seo:check:prod` confirms `application/manifest+json` or `application/json`; otherwise the remaining fix is nginx/server MIME config. |
...
- `PUBLIC-E2E-010` follow-up implementation 07.06.2026: `robots.txt` was checked and left unchanged; `.htaccess` now maps `.webmanifest` to `application/manifest+json`; `seo-check.mjs` guards the local MIME hint and production manifest `Content-Type`. Local `node --check tools/seo-check.mjs` and `npm run seo:check` passed. Production `seo:check:prod` is the closure gate after deploy/cache refresh.
