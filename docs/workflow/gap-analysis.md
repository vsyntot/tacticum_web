...
If future work starts from the 07.06 `/about/` challenge, convert remaining local `ABOUT-*` observations into tracker issues using `about-page-ux-content-challenge-issue-backlog-2026-06-07.md`. `ABOUT-WP-01`, `ABOUT-WP-02` and `ABOUT-WP-08` are closed for fast-fix/guard scope after production evidence; reopen them only on regression. Next non-regression implementation should start from `ABOUT-WP-04` team UI/accessibility, or from owner decisions on the prepared `ABOUT-WP-03` proof matrix and `ABOUT-WP-07` ownership map. `ABOUT-WP-05` and `ABOUT-WP-06` can continue only inside the approved trust/ownership boundaries. Do not rewrite trust claims, team/person data, public proof, footer SEO semantics, form IDs/payloads or page-content/fallback ownership without the owner gates listed in the roadmap and backlog. Do not fix stale timeline regressions by simply changing the year; separate historical milestones from current operating focus.
