...
07.06.2026 `/about/` team UI/accessibility slice implemented locally for `ABOUT-WP-04` / `ABOUT-005`: team cards no longer depend on hover-only `.member-overlay` for readable detail, existing approved team data is shown in normal card flow, contact links expose accessible labels, and obsolete overlay JS/CSS was removed. `PHOTO` is now explicitly requested by the `values-team` content list params. This does not change personal/team data or close `ABOUT-005`; browser/mobile smoke, production deploy/cache clear and rendered evidence are still required.
