...
07.06.2026 `/about/` team UI/accessibility slice has production rendered/source evidence for `ABOUT-WP-04` / `ABOUT-005`: team cards no longer depend on hover-only `.member-overlay` for readable detail, existing approved team data is shown in normal card flow, contact links expose accessible labels, and obsolete overlay JS/CSS was removed. `PHOTO` is now explicitly requested by the `values-team` content list params. Production `content:public-cache-clear` completed; `content:public-hygiene:rendered:prod:json` passed at `2026-06-07T10:01:10Z` with `pages_checked=13`, `issues_found=0`; `page-content:source:http:wave2:prod` confirmed `/about/ source=bitrix sections=3/3 bytes=77945`. This does not change personal/team data or fully close `ABOUT-005`; browser/mobile visual smoke remains required.
