...
Этот слой не о file-size/componentization, а о смысловом владении контентом. Product content migration закрыла редактируемость продуктовых страниц через `products`, `product_blocks` and `product_use_cases`, но не закрыла финальное распределение доменных сущностей по existing iblocks. Local implementation 05.06.2026 закрыл кодовый слой для product FAQ first-read из `faq #10`, `clients` config registry, `feedback/clients` relation foundation, services fallback removal and governance tooling. Production evidence 05.06.2026 подтвердил FAQ seed/audit, product `faq_source=iblock` after cache clear, product FAQ section sync, services seed/audit, proof strict audit, `feedback/clients` relation apply and Chrome-capable rendered/action smoke. Production evidence 06.06.2026 подтвердил owner-approved proof tagging and public-render apply: 17 active proof items reviewed, 2 PRODUCT relations aligned to approval, 12 `PUBLIC_RENDER_APPROVED=Y` flags applied, `clients` remain global, strict proof audit reports `public_proof_render_ready=true` for all four products, and product HTTP source smoke passed with `proof_source=iblock`. FAQ fallback retirement approval passed with `retirement_allowed=true` and owner gates `4/4`; after deploy, product cache clear, strict FAQ audit, strict product content check and HTTP source smoke passed with `faq_source=iblock` for all product pages. `content-storage-proof-tagging-helper.php` provides an internal owner-review worksheet, `content-storage-proof-approval-template.php` generates a no-raw-copy blank approval draft and `content-storage-proof-tagging-proposal.php` generates a proposed product-tagging draft on production without `/docs`, `content-storage-proof-approval-check.mjs` validates no-raw-copy owner decisions, `content-storage-proof-tagging-apply.php` provides approved dry-run/apply of `PRODUCT` tags without changing proof copy or public rendering, and `content-storage-proof-public-render-apply.php` provides the separate durable `PUBLIC_RENDER_APPROVED` gate after current PRODUCT verification. `/agents/` vs `/aiagents/` now has a static boundary guard: product Agents stays under product content and `/aiagents/` stays the demo/prototype service route; production aiagents tagging applied `PRODUCT=agents` to active demo-agent rows #523, #524 and #525 without public rendering changes; strict aiagents audit reports `agents aiagents_items=3` and zero items for other products. Production server visual/browser smoke remains environment-blocked by missing Chrome/Chromium; Chrome-capable targeted visual/action smoke for changed URLs `/platform/`, `/agents/`, `/dev/`, `/forum/`, `/aiagents/` passed from local environment after broad-run automation timeouts were isolated as CDP/tooling timeouts without network/console/page errors. Structured page-content now has a guarded draft model for `page_sections/page_blocks`, a schema-only approved model artifact, approved-only schema migration CLI, seed CLI, live-status approval/apply tooling, live-only renderer foundation and `page-content` audit scope; production dry-run, schema apply, wave 1 shadow seed, live-status apply and source switch passed on 06.06.2026: `page_sections #24`, `page_blocks #25`, 25 properties, config registry update, 9 active live sections, 37 active blocks, `orphan_blocks=0`, `page_content.source=bitrix`, HTTP source markers `/services/ 3/3`, `/price/ 2/2`, `/contacts/ 2/2`, `/offer/ 2/2`, strict audit clean and `seo:check:prod` passed. Post-switch targeted Chrome-capable visual/browser smoke for the switched pages passed locally. Page-content fallback retirement now has a no-raw-copy template/checker gate with scoped wave 2 support; production owner-approved wave 1 fallback-retirement check passed on 06.06.2026 with `retirement_allowed=true`, production evidence `9/9`, owner gates `5/5` and 9 `retire_fallback` decisions. Local code retirement removed fallback section bodies from the nine wave 1 partials and governance now forbids returning static `<section>` fallback bodies there. Post-deploy page-content fallback-retirement source/audit/SEO/browser rechecks passed; page-content and proof public-render closure are complete for the current production scope.
...
| Cluster | Gap IDs | Current Risk |
|---|---|---|
| Domain iblock placement | `CSG-001` - `CSG-006` | Product FAQ/services/clients/relation/proof public rendering and `/agents/` vs `/aiagents/` boundary guard are in place; future changes remain owner-gated |
| Page section content | `CSG-007`, `CSG-008`, `CSG-012` | Wave 1 and wave 2 page-level sections render from `page_sections/page_blocks` on production; approved fallback bodies are retired/rechecked; narrow iblocks remain protected |
| Guards / release evidence | `CSG-009` - `CSG-011` | Audit/governance/runbook exists; FAQ/services/proof/aiagents production evidence passed; page-content source HTTP/audit/SEO and targeted Chrome-capable visual/browser evidence passed after switch and retirement |
| Page-section model | `CSG-007`, `CSG-008`, `CSG-012` | Draft and approved `page_sections/page_blocks` models, checker, migration CLI, seed/live tooling, renderer foundation, audit scope and fallback-retirement checker exist; production source switch and approved fallback retirement are active/rechecked for both waves |
...
- `content:storage:governance:check` protects the content-storage boundary.
- `content-storage-audit.php` provides aggregate, no-PII evidence for iblock counts, relation coverage and per-product proof relation counts.
- `content-storage-faq-migration.php` seeded product FAQ rows into `faq` with `PRODUCT` relation on production; strict FAQ audit passed with 3 related FAQ items per product.
- Product cache clear, strict product content check and HTTP source smoke passed with `faq_source=iblock` for all four product pages.
- FAQ fallback retirement approval passed with `retirement_allowed=true`, owner gates `4/4`, and post-deploy FAQ/product source checks passed with `faq_source=iblock`.
- Production page-content dry-run passed on 06.06.2026 before apply: planned `tacticum_page_sections`, `tacticum_page_blocks` and 25 properties with no seed or runtime switch.
- Page-content schema-only approved model artifact is available in `content-storage-page-content-model-2026-06-06.approved.json`; it permits empty schema creation only and does not approve page copy seed, public runtime switch or fallback retirement.
- Production page-content schema apply passed on 06.06.2026: `page_sections #24`, `page_blocks #25`, 25 properties, config registry update and strict page-content audit passed.
- Production wave 1 page-content shadow seed passed on 06.06.2026 for `/services/`, `/price/`, `/contacts/` and `/offer/`: 9 active sections, 37 active blocks and `orphan_blocks=0`. Production live-approval check and live-status apply promoted all 9 sections to `MIGRATION_STATUS=live`; pre-switch fallback HTTP source check passed with zero Bitrix markers. After the explicit environment switch to `page_content.source=bitrix`, runtime config check, page-content source HTTP check, strict page-content audit and `seo:check:prod` passed; `/services/`, `/price/`, `/contacts/` and `/offer/` now report Bitrix-rendered section counts `3/3`, `2/2`, `2/2` and `2/2` respectively.
- Production wave 2 page-content shadow seed and scoped live apply passed on 06.06.2026 for `/`, `/about/`, `/calculator/` and `/aiagents/`: 11 sections, 43 blocks, strict audit with 20 active sections, 80 active blocks, all wave 2 sections `live` and `orphan_blocks=0`; scoped source HTTP check passed with Bitrix markers after the `/calculator/` template correction.
- Page-content live-status promotion remains owner-gated by `content-storage-page-content-live-approval-template.php`, `content-storage-page-content-live-approval-check.mjs` and `content-storage-page-content-live-apply.php`; these tools approve/apply only `page_sections.MIGRATION_STATUS`. The source switch is a separate environment/config action, already explicit as `page_content.source=bitrix` for the current production environment. Fallback retirement is guarded by `content-storage-page-content-fallback-retirement-template.php` and `content-storage-page-content-fallback-retirement-check.mjs`, including scoped wave 2 approval files; production wave 1 approval passed with production evidence `9/9` and owner gates `5/5`, and wave 2 approval passed with production evidence `9/9` and owner gates `5/5`. The checker itself does not remove files; code retirement removed the approved wave 1 and wave 2 fallback bodies after their respective owner-approved deploys.
- FAQ section sync passed on production: sections `products/platform/agents/dev/forum` were created/used, 12 existing FAQ rows were linked and strict audit reports `faq_items_without_section=0`.
- `content-storage-services-seed.php` seeded six target delivery-package cards into `services` on production; strict services audit passed with active/API count `6`.
- `content-storage-audit.php --scope=proof --strict --json` passed on production for `clients`, `feedback`, `cases` and all relevant `PRODUCT` relations; local audit contract now also reports public-render proof counts.
- Owner-approved proof tagging and public-render apply passed on production: `cases #181` and `feedback #92` PRODUCT tags were aligned to approval, 12 `PUBLIC_RENDER_APPROVED=Y` flags were applied, strict proof audit reports `public_proof_render_ready=true` for all products, and product HTTP source smoke passed with `proof_source=iblock`.
- Proof public rendering is active on production: `content-storage-proof-public-render-apply.php` verifies current PRODUCT tags, changes only `PUBLIC_RENDER_APPROVED`, product runtime reads `cases/feedback/clients` only through `PROPERTY_PRODUCT + PUBLIC_RENDER_APPROVED=Y`, and product source HTTP evidence requires `proof_source=iblock` for the current release.
- `content:storage:aiagents-boundary:check` guards `/agents/` vs `/aiagents/` source and positioning boundary; production aiagents tagging applied `PRODUCT=agents` to active rows #523, #524 and #525, and strict audit reports `agents aiagents_items=3`.
- `content:storage:page-content-model:check` validates the draft model and rejects narrow iblocks/raw HTML/JSON blob storage for page sections.
- `content-storage-page-content-migration.php` dry-runs the `page_sections/page_blocks` schema and rejects `--apply` until an approved owner-gated model is supplied; `content-storage-page-content-seed.php` seeds/updates wave 1 and wave 2 rows and preserves an existing non-empty `MIGRATION_STATUS` during updates; `content-storage-page-content-live-approval-check.mjs` validates no-raw-copy live-status approvals; `content-storage-page-content-live-apply.php` changes only `MIGRATION_STATUS`; live runtime foundation reads Bitrix only when `page_content.source=bitrix` and section status is `live`; `content-storage-audit.php --scope=page-content --strict --json` validates schema readiness and aggregate row evidence after apply.
- `seo:check:prod`, Chrome-capable `visual:smoke:prod` and Chrome-capable `browser:smoke:prod` passed from local environment.
...
- decision baseline: `docs/workflow/offer-page-taxonomy-presets-decision-2026-06-07.md`;
- owner approval draft: `docs/workflow/offer-taxonomy-presets-owner-approval-2026-06-07.draft.json`;
- owner approval: `docs/workflow/offer-taxonomy-presets-owner-approval-2026-06-07.approved.json`;
...
- runtime implementation plan: `docs/workflow/plans/2026-06-07-offer-taxonomy-runtime-implementation.md`.
...
| Cluster | Gap IDs | Current Risk |
|---|---|---|
| Public taxonomy ownership | `OFFER-TAX-001`, `OFFER-TAX-003`, `OFFER-TAX-005`, `OFFER-TAX-011` | Runtime source is now governed by Bitrix taxonomy terms; synthetic data positioning and budget-bucket governance remain separate owner/product questions. |
| Quick entries / presets | `OFFER-TAX-002` | Closed for featured-term runtime source: quick entries are curated and runtime can read featured terms from `offer_taxonomy_terms`; future richer preset rows remain a separate decision if needed. |
| Bitrix content model | `OFFER-TAX-006` | Closed for current WP-04 runtime/source-switch scope: `ADR-012`, approved owner JSON, `offer_taxonomy_terms #28`, strict `source=bitrix`, fallback disabled, cache clear and rendered/SEO evidence passed. |
...
| SEO / guards / scaling | `OFFER-TAX-007`, `OFFER-TAX-009`, `OFFER-TAX-010`, `OFFER-TAX-012` | Filtered URL SEO is currently safe and accepted-monitor; raw-budget/first-8 guards, taxonomy checker/cache-clear tooling and strict production source-switch evidence passed; PHP cached-array filtering is acceptable for current volume but monitored. |
...
- Interim `OFFER-TAX-WP-02` started without Bitrix schema: quick entries render curated active keys via `CatalogTaxonomy::featuredOptions()` instead of first sorted aggregated options. WP-04 then moved featured-term ownership into strict Bitrix runtime via `offer_taxonomy_terms #28`.
...
- `OFFER-TAX-WP-03` is owner-approved: accepted `ADR-012`, approved JSON and `offer:taxonomy:approval:*` scripts validate owners, gates, labels, aliases, featured terms, budget policy, no stored counts and no runtime source switch/iblock apply approval.
- `OFFER-TAX-WP-04` runtime/source-switch scope is complete from the approved JSON/model: `OfferTaxonomyService` owns fallback/auto/Bitrix source selection, alias canonicalization, public labels and featured terms; `offer_taxonomy_terms #28` is configured as the strict Bitrix source; migration/check/cache-clear scripts exist; `offer_filter_presets` is not created because approved preset source is `featured_terms`.
- Local verification passed for static gates: PHP lint for changed offer PHP files, JS syntax for hygiene tools, `content:public-hygiene:self-test`, `content:public-hygiene:rendered:self-test`, `content:public-hygiene:check`, `seo:check`, `bitrix:check`, `config:check`, `offer:taxonomy:implementation-gate`, PHP output smoke snippets and `git diff --check`. Local `offer:taxonomy:check` and `offer:taxonomy:migrate` are Bitrix DB-blocked by missing local MySQL socket and must run on target/prod.
- Production cache clear and rendered evidence passed for the deployed fast-fix baseline: `content:public-hygiene:rendered:prod:json` at `2026-06-07T12:22:16Z` reports `pages_checked=13`, `issues_found=0`, `/offer/ ok=true`; `page-content:source:http:prod` reports `/offer/ source=bitrix sections=2/2 bytes=162309`; `seo:check:prod` passed. WP-04 target schema/seed/readability evidence also passed: dry-run/apply created `offer_taxonomy_terms #28`, and fallback-mode `offer:taxonomy:check` reports `source=fallback`, `active=22`, `featured=16`, `seen=22`, `unknown_dimension=0`. Soft `auto` runtime smoke then passed with runtime `source=bitrix`, fallback still enabled, cache clear managed tags `iblock_id_5`, `iblock_id_28`, and rendered hygiene JSON at `2026-06-07T14:16:57Z` reports `pages_checked=13`, `issues_found=0`. Final strict switch passed with `source=bitrix`, `fallback=false`, 22 active terms, 16 featured terms, rendered hygiene JSON at `2026-06-07T14:19:19Z` reports `pages_checked=13`, `issues_found=0`, and `seo:check:prod` passed. `OFFER-TAX-WP-04` is closed for runtime/source-switch scope; `OFFER-TAX-WP-06` remains product/SEO gated.
...
Production is stable after the public E2E hygiene deploy. Public pages return 200, Chrome-free HTTP/SEO/content checks pass, rendered content hygiene is clean, SEO guards pass, robots/sitemaps are reachable, `/offer/sitemap.php` has 1118 live 200 URLs, and page/product content sources are Bitrix. The immediate release-hygiene gaps for missing `lang=ru`, stale generated Tailwind artifact, stale static sitemap `lastmod`, stale sitemap index lastmods and webmanifest MIME are closed. `robots.txt` was rechecked and left unchanged because it already points to the HTTPS sitemap. Remaining product/content gaps are mostly owner-gated: heavy English/internal terminology, `/price/` public label governance, `/offer/` synthetic example disclosure, supporting-page polish and accepted monitoring for `/aiagents/` vs `/agents/` plus CSP report-only. Production-server Chrome-based smoke is still blocked by missing Chrome/Chromium on that host, so browser evidence must run from a Chrome-capable local/CI runner or after configuring `CHROME_PATH`.

### Current Gap Coverage

| Cluster | Gap IDs | Current Risk |
|---|---|---|
| Technical release hygiene | `PUBLIC-E2E-001`, `PUBLIC-E2E-002`, `PUBLIC-E2E-003`, `PUBLIC-E2E-014` | Closed for HTTP/SEO/content scope: `lang` rendering and guards are deployed, production `release:public-precheck:prod` confirms `lang=ru` for 13/13 pages and now checks robots, sitemap index, static sitemap, offer sitemap and webmanifest; Tailwind artifact is regenerated, static sitemap artifact is regenerated and production `seo:check:prod` passed. `PUBLIC-E2E-002` keeps only a server-Chrome caveat because the production host lacks Chrome/Chromium for visual/browser automation. |
| SEO infrastructure | `PUBLIC-E2E-004`, `PUBLIC-E2E-010` | Closed: `PUBLIC-E2E-004` closed by sitemap index lastmod update and production `seo:check:prod`; `PUBLIC-E2E-010` closed by `.webmanifest` MIME mapping and production `seo:check:prod` manifest `Content-Type` guard. |
...
- Production verification passed after deploy/cache refresh: `content:public-cache-clear`, `seo:check:prod`, `release:public-precheck:prod` and `content:public-hygiene:rendered:prod:json` passed; rendered hygiene JSON at `2026-06-07T20:11:21Z` reported `pages_checked=13`, `issues_found=0`, and `release:public-precheck:prod` confirmed `lang=ru` for all 13 public pages.
- Production-server `visual:smoke:prod` and `browser:console:prod` are environment-blocked by missing Chrome/Chromium. This is not a site regression; use a Chrome-capable local/CI runner or install Chrome/Chromium and set `CHROME_PATH`.
- `PUBLIC-E2E-010` follow-up closure 07.06.2026: `robots.txt` was checked and left unchanged; `.htaccess` now maps `.webmanifest` to `application/manifest+json`; `seo-check.mjs` guards the local MIME hint and production manifest `Content-Type`. Local `node --check tools/seo-check.mjs` and `npm run seo:check` passed. Production `content:public-cache-clear`, `seo:check:prod` and `content:public-hygiene:rendered:prod:json` passed after deploy; rendered hygiene JSON at `2026-06-07T20:37:59Z` reported `pages_checked=13`, `issues_found=0`.
- `PUBLIC-E2E-WP-05` release-precheck consolidation closure 07.06.2026: `release-public-precheck.mjs` now performs HTTP-only checks for `robots_txt`, `sitemap_index`, `static_sitemap`, `offer_sitemap` and `webmanifest`; production `npm run release:public-precheck:prod` passed with `robots_txt status=200 sitemap=ok`, `sitemap_index status=200 sitemaps=2`, `static_sitemap status=200 urls=13 missing=0`, `offer_sitemap status=200 urls=1118` and `webmanifest status=200 type=application/manifest+json`.
...
- `/about/` UX/content source register: `docs/workflow/about-page-ux-content-challenge-gap-analysis-2026-06-07.md`;
- `/about/` UX/content roadmap: `docs/workflow/about-page-ux-content-challenge-roadmap-2026-06-07.md`;
- `/about/` UX/content issue backlog: `docs/workflow/about-page-ux-content-challenge-issue-backlog-2026-06-07.md`;
- `/about/` guard proposal: `docs/workflow/about-page-ux-content-challenge-guard-proposal-2026-06-07.md`;
- Codex plan for the `/about/` challenge snapshot: `docs/workflow/plans/2026-06-07-about-page-ux-content-challenge-documentation.md`;
- `/about/` owner-review proof matrix: `docs/workflow/about-page-proof-matrix-owner-review-2026-06-07.md`;
- `/about/` source ownership map: `docs/workflow/about-page-content-ownership-map-2026-06-07.md`;
- Codex plan for the `/about/` proof/ownership owner-review package: `docs/workflow/plans/2026-06-07-about-owner-review-proof-ownership.md`;
- Codex plan for the `/about/` team UI/accessibility slice: `docs/workflow/plans/2026-06-07-about-team-ui-accessibility-slice.md`.
- `PTC-WP-01` target evidence refresh: `docs/workflow/product-content-target-evidence-refresh-2026-06-07.md`.
...
07.06.2026 `/about/` UX/content challenge зафиксировал отдельный page-specific register. Local IDs `ABOUT-001` - `ABOUT-010` cover stale `2025 / Сегодня` timeline, fragmented trust storyline, Russian-first/internal terminology debt, weak enterprise trust proof, team UI/accessibility, generic technology stack, duplicate IDs and broken/misleading anchors, CTA/career mismatch, page-content/fallback ownership ambiguity and missing guard coverage. These IDs are not canonical product-tech gap IDs; they map back to existing `CLS-*`, `CONTENT-*`, `UX-*`, `UI-*`, `CSG-*`, `BPC-*`, `STACK-*` and `REL-*` gaps. Use `ABOUT-WP-01` - `ABOUT-WP-08` for future `/about/` implementation planning.

07.06.2026 `/about/` fast-fix/guard implementation completed production evidence for `ABOUT-WP-01`, `ABOUT-WP-02` and `ABOUT-WP-08`: source partials, footer menu, page-content seed, `PublicCopyNormalizer`, source hygiene and rendered hygiene self-tests were updated; production `content:public-cache-clear` completed; `content:public-hygiene:rendered:prod:json` passed at `2026-06-07T08:24:11Z` with `pages_checked=13`, `issues_found=0`; `page-content:source:http:wave2:prod` confirmed `/about/ source=bitrix sections=3/3`. This closes local `ABOUT-001`, `ABOUT-007` and `ABOUT-010` for fast-fix/guard scope. `ABOUT-003`, `ABOUT-006` and `ABOUT-008` remain partial/in-progress; `ABOUT-002`, `ABOUT-004`, `ABOUT-005` and `ABOUT-009` remain owner-gated.

07.06.2026 `/about/` trust-storyline slice advanced `ABOUT-WP-03`, `ABOUT-WP-05` and `ABOUT-WP-06`: hero/company copy, stat cards, technology/stack copy, final CTA, footer start-work anchor, wave2 seed, context-aware `/about/` page-content normalization and source/rendered guards now frame the page around corporate AI launch responsibility without adding proof claims. Local PHP lint, `product:content:safety:check`, `seo:check`, `bitrix:check` and `git diff --check` passed. Production `content:public-cache-clear` completed; `content:public-hygiene:rendered:prod:json` passed at `2026-06-07T08:49:12Z` with `pages_checked=13`, `issues_found=0`; `page-content:source:http:wave2:prod` confirmed `/about/ source=bitrix sections=3/3 bytes=76054`. Keep `ABOUT-002`, `ABOUT-003`, `ABOUT-006` and `ABOUT-008` in-progress until owner decisions exist; `ABOUT-004` remains blocked for claims, `ABOUT-005` remains open for team UI, and `ABOUT-009` remains owner-gated content ownership work.

07.06.2026 `/about/` proof/ownership owner-review package created: `about-page-proof-matrix-owner-review-2026-06-07.md` defines no-raw-copy public/private/blocked trust statement categories for `ABOUT-004`, and `about-page-content-ownership-map-2026-06-07.md` maps actual `/about/` render ownership across hardcoded PHP partials, Bitrix page-content rows, team iblock, lead CTA, footer menu and `PublicCopyNormalizer` compatibility. This does not close `ABOUT-004` or `ABOUT-009`; public proof claims, Bitrix row ownership changes and fallback retirement remain owner-gated.

07.06.2026 `/about/` team UI/accessibility slice has production rendered/source and browser/mobile evidence for `ABOUT-WP-04` / `ABOUT-005`: team cards no longer depend on hover-only `.member-overlay` for readable detail, existing approved team data is shown in normal card flow, contact links expose accessible labels, and obsolete overlay JS/CSS was removed. `PHOTO` is now explicitly requested by the `values-team` content list params. Production `content:public-cache-clear` completed; `content:public-hygiene:rendered:prod:json` passed at `2026-06-07T10:01:10Z` with `pages_checked=13`, `issues_found=0`; `page-content:source:http:wave2:prod` confirmed `/about/ source=bitrix sections=3/3 bytes=77945`. Chrome-capable visual smoke for `/about/` passed at `2026-06-07T10:05:26Z` for desktop and mobile with status `200`, runtime errors `0`, warnings `0`, broken images `0`, action errors `0`, SEO ok; manifest is `/tmp/tacticum-about-team-ui-2026-06-07-visual/manifest.json`. This does not change personal/team data; PM/Design visual acceptance remains the only full-closure gate for `ABOUT-005`.

07.06.2026 `/about/` timeline marker UI follow-up is deployed with production evidence: history markers now separate quiet past years, bordered `2025` key milestone and primary current focus with explicit `Сейчас` badge. This addresses a follow-up UX concern where filled blue `2025` looked more current than the dark `Сейчас` marker. No claims, Bitrix rows, forms, routes, SEO metadata or team data changed. Local PHP lint, `css:syntax`, `content:public-hygiene:check` and `seo:check` passed. Production `content:public-cache-clear` completed; `content:public-hygiene:rendered:prod:json` passed at `2026-06-07T10:33:14Z` with `pages_checked=13`, `issues_found=0`; `page-content:source:http:wave2:prod` confirmed `/about/ source=bitrix sections=3/3 bytes=77870`. Chrome-capable visual smoke for `/about/` passed at `2026-06-07T10:34:10Z` for desktop and mobile with status `200`, runtime errors `0`, warnings `0`, broken images `0`, action errors `0`, SEO ok; manifest is `/tmp/tacticum-about-timeline-marker-2026-06-07-visual/manifest.json`. PM/Design visual acceptance remains pending for full design-gated Phase 3 closure.

07.06.2026 `/about/` work-model feature-grid follow-up is deployed with production evidence: the `values-team` block no longer has to render as contextless generic value cards. `feature-card-grid` now renders section heading/intro, uses count-aware grid density and static process-card CSS; `/about/` copy is reframed as pre-launch operating model across scenario, responsibility and constraints. `PublicCopyNormalizer` protects old and current Bitrix block keys, so no manual Bitrix row edit is required for old live strings. Local PHP lint, render fixture, CSS/template checks, content hygiene, rendered self-test, SEO, Bitrix architecture and component state checks passed. Production `content:public-cache-clear` completed; `content:public-hygiene:rendered:prod:json` passed at `2026-06-07T10:52:28Z` with `pages_checked=13`, `issues_found=0`; `page-content:source:http:wave2:prod` confirmed `/about/ source=bitrix sections=3/3 bytes=77902`. Chrome-capable visual smoke for `/about/` passed at `2026-06-07T10:53:28Z` for desktop and mobile with status `200`, runtime errors `0`, warnings `0`, broken images `0`, action errors `0`, SEO ok; manifest is `/tmp/tacticum-about-work-model-feature-grid-2026-06-07-visual/manifest.json`. PM/Design visual acceptance remains pending for full design-gated Phase 3 closure.
...
If future work starts from the 07.06 `/about/` challenge, convert remaining local `ABOUT-*` observations into tracker issues using `about-page-ux-content-challenge-issue-backlog-2026-06-07.md`. `ABOUT-WP-01`, `ABOUT-WP-02` and `ABOUT-WP-08` are closed for fast-fix/guard scope after production evidence; reopen them only on regression. `ABOUT-WP-04` has production rendered/source/browser evidence and needs only PM/Design visual acceptance before full closure. Next non-regression implementation should start from owner decisions on the prepared `ABOUT-WP-03` proof matrix and `ABOUT-WP-07` ownership map, or continue `ABOUT-WP-05`/`ABOUT-WP-06` only inside approved trust/ownership boundaries. Do not rewrite trust claims, team/person data, public proof, footer SEO semantics, form IDs/payloads or page-content/fallback ownership without the owner gates listed in the roadmap and backlog. Do not fix stale timeline regressions by simply changing the year; separate historical milestones from current operating focus.
...
PTC-WP-01 evidence refresh 07.06.2026: local `product:content:safety:check` passed and HTTP production product-source/release precheck passed with product pages rendering `source=bitrix`, `faq_source=iblock`, `proof_source=iblock` and 11 blocks. This refresh confirms the public/product HTTP layer is healthy after the latest deploy cycle, but it does not close `CFG-001`, `CFG-002`, `CFG-003`, `ARCH-001`, `ARCH-003` or `STACK-004`: local `product:content:cache-clear:dry-run:json` cannot run without target Bitrix DB access and failed on local MySQL socket absence. Target strict JSON evidence, target-evidence validation, cache-clear dry-run JSON and cache-clear evidence validation remain pending under `PTC-WP-01`.
