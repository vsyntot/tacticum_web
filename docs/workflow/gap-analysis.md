...
Release evidence refresh 03.06.2026: regenerated current production smoke manifests for rendered SEO, warning-aware browser actions, focused `/price/` team preset smoke and CSS-local visual/action checks under `/tmp/tacticum-release-closure-2026-06-03/`. Updated `release-signoff-2026-05-24-post-deploy.draft.json` now passes `release:signoff:summary` and `release:signoff:draft-check`; this closes the local stale-manifest tail. Strict release closure remains blocked only by external/manual gates `S12-001` - `S12-004` and legacy full-window inventory `S12-006`.

Manual release gates helper 03.06.2026: `tools/release-manual-gates-helper.mjs` / `npm run release:manual-gates:helper` closes the local handoff-format gap for the remaining manual gates. It reads the current draft sign-off when available, prints pending `manual-success-flow`, `metrika-goals`, `bitrix-admin` and `staff-sale-upstream` by default, and gives owner-specific next actions plus safe evidence skeletons that match `release-signoff-check.mjs`. If `docs/` is absent on production, it falls back to standalone skeleton mode instead of failing on missing draft JSON. Production server run from `/home/bitrix/www` on 03.06.2026 confirmed default, focused `staff-sale-upstream` and JSON outputs in standalone mode. Bitrix admin was later closed by owner evidence; Metrika access and CRM/upstream IDs still require external owner evidence.

Manual success-flow helper 03.06.2026: `tools/manual-success-flow-helper.mjs` / `npm run manual:success-flow:helper` closes the local helper gap for owner-run default form, modal form, AI chat and prefill checks. It generates controlled payloads, browser snippet, curl templates and evidence skeleton without sending requests; staff-order remains delegated to `staff:sale:gate-helper` because it has its own rich workers/upstream contract. Production server run from `/home/bitrix/www` on 03.06.2026 confirmed default, browser, curl and release helper outputs. This does not close `S12-002`: real successful form/chat/upstream evidence or accepted controlled error states still require owner-run checks and safe IDs.

Metrika goals helper 03.06.2026: `tools/metrika-goals-helper.mjs` / `npm run metrika:goals:helper` closes the local helper gap for `S12-004`. It prints expected goals/events, checks deployed JS taxonomy for event presence and Metrika counter wiring, emits a browser observer snippet for `tacticum:analytics` events, and gives a safe evidence skeleton. Production server run from `/home/bitrix/www` on 03.06.2026 confirmed default, source-check/evidence, browser and release helper outputs; source-check found `missing_events: -`, counter `103471113`, `reachGoal`, QA browser event and HTTPS Metrika tag. This does not close `S12-004`: only PM/Marketing + QA with Yandex.Metrika/tag-manager access can confirm actual goal visibility and no-PII params.
...
Staff-sale upstream gate hardening 03.06.2026: `tools/staff-sale-gate-helper.mjs` and `npm run staff:sale:gate-helper` now generate a controlled staff-order payload, curl template and safe evidence block for the next owner-run `staff-sale-upstream` check. `release-signoff-check.mjs` now requires `team_preset`, `workers_count`, `monthly_budget_estimate_present=true`, `end_date_present=true` and `upstream_request_id`/`lead_id` before the gate can pass. Browser-controlled `/price/` staff-order POST returned HTTP 200 `success=true` on 03.06.2026 for a controlled payload with `team_preset=mvp`, `workers_count=3`, monthly budget and exact end date. This narrows `S12-003` from endpoint recovery to CRM/upstream evidence collection, but does not close it until safe `lead_id` or `upstream_request_id` exists.
...
| S14-004 | external handoff | External evidence | `manual-success-flow`, `metrika-goals`, `staff-sale-upstream`, legacy access logs/CRM inventory и strict sign-off остаются внешними gates с owner/due/evidence rules |
...
Вывод challenge: сайт близок к целевому состоянию. Sprint 16 локально закрыл code/docs gaps по карте `/contacts/`, contrast offer detail, calculator/price chat-to-lead handoff, CTA image trust, proof matrix, SEO/CSP decisions и contact/legal hierarchy. Production deploy/cache smoke 25.05.2026 прошёл по rendered/action/price gates, включая новую карту `Тактикум` на `/contacts/`. Staff-order endpoint slice восстановлен: 03.06.2026 browser-controlled `/price/` POST returned HTTP 200 `success=true`, а Bitrix admin smoke закрыт owner evidence. Сайт всё ещё нельзя считать полностью стабилизированным до восстановления остальных upstream success-flow, CRM/upstream staff-order evidence и подтверждения Metrika goals.
...
| FSC-004 | external blocker | P1 | Real success-flow / staff upstream | 25.05.2026 controlled production smoke attempted; default form, modal form and AI chat returned upstream `502`, prefill controlled empty state returned expected `404`; 03.06.2026 browser-controlled staff-order POST returned HTTP 200 `success=true`, CRM/upstream ID evidence still pending |
