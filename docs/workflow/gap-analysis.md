...
| PG-008 | in-progress | P2 | Full Feature | Layout consistency | Personal-offer CTA и project-discussion CTA вынесены в includes, но contacts CTA ещё требует отдельного решения | `local/templates/tacticum/include/personal-offer-cta.php`, `local/templates/tacticum/include/project-discussion-cta.php`, `index.php`, `calculator/index.php`, `price/index.php`, `about/index.php`, `services/index.php`, `contacts/index.php` | UX-правки основных CTA теперь делаются в одном месте; contacts CTA всё ещё может расходиться | Отдельно нормализовать contacts CTA variant после visual review |
...
| TG-017 | in-progress | P1 | Full Feature | JS-owned markup | Specialist modal markup перенесён из JS в Bitrix component template; основные repeated CTA вынесены в includes; остаются прочие крупные layout blocks | `news.list/price/template.php`, `news.list/price/script.js`, `modal.js`, `local/templates/tacticum/include/personal-offer-cta.php`, `local/templates/tacticum/include/project-discussion-cta.php`, public pages | Заказ специалистов и основные CTA стали ближе к Bitrix component/include pattern; крупные публичные sections всё ещё требуют inventory | Продолжить componentization для contacts CTA и прочих повторяемых layout variants |
