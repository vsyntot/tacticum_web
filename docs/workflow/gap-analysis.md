...
| TG-028 | closed | P2 | Full Feature | CSS retirement governance | Безопасный план вывода legacy `template_styles.css` зафиксирован; прямое удаление отложено до ownership mapping и production visual baseline | `template-styles-retirement-plan.md`, `static-css-build-plan.md`, `asset-layout-audit.md` | CSS cleanup получил контролируемый маршрут без одномоментного риска снова сломать дизайн сайта | Реализовывать migration малыми партиями после каждого чистого post-deploy smoke |
