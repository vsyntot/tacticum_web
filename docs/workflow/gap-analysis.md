...
| Page section content | `CSG-007`, `CSG-008`, `CSG-012` | Wave 1 page-level sections now render from `page_sections/page_blocks` on production; narrow iblocks remain protected; PHP partials stay deployed as rollback fallback; wave 2 remains PHP partial content |
...
| Page-section model | `CSG-007`, `CSG-008`, `CSG-012` | Draft and approved `page_sections/page_blocks` models, checker, migration CLI, seed/live tooling, renderer foundation, audit scope and fallback-retirement checker exist; production source switch and approved fallback retirement are active/rechecked for wave 1; wave 2 remains PHP partial content |
...
- `content-storage-audit.php --scope=proof --strict --json` passed on production for `clients`, `feedback`, `cases` and all relevant `PRODUCT` relations.
- Owner-approved proof tagging apply passed on production: `platform proof_items_total=6`, `agents proof_items_total=6`, `dev proof_items_total=6`, `forum proof_items_total=5`; public product proof rendering remains blocked because `public_render_approved=false`.
- Proof owner decisions now have a checked approval JSON contract before any public proof implementation.
...
- `content-storage-page-content-migration.php` dry-runs the `page_sections/page_blocks` schema and rejects `--apply` until an approved owner-gated model is supplied; `content-storage-page-content-seed.php` seeds wave 1 rows only in shadow mode; `content-storage-page-content-live-approval-check.mjs` validates no-raw-copy live-status approvals; `content-storage-page-content-live-apply.php` changes only `MIGRATION_STATUS`; live runtime foundation reads Bitrix only when `page_content.source=bitrix` and section status is `live`; `content-storage-audit.php --scope=page-content --strict --json` validates schema readiness and aggregate row evidence after apply.
