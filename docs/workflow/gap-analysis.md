...
| ID | Status | Area | Closure Target |
|---|---|---|---|
| S12-001 | external handoff | Release sign-off closure | Manual gates closed; strict release sign-off now requires final release commit marker instead of `working-tree-*` plus legacy full-window/source inventory decision |
...
| S14-004 | external handoff | External evidence | legacy access logs/CRM inventory и strict sign-off остаются внешними gates с owner/due/evidence rules |
...
Вывод challenge: сайт близок к целевому состоянию. Sprint 16 локально закрыл code/docs gaps по карте `/contacts/`, contrast offer detail, calculator/price chat-to-lead handoff, CTA image trust, proof matrix, SEO/CSP decisions и contact/legal hierarchy. Production deploy/cache smoke 25.05.2026 прошёл по rendered/action/price gates, включая новую карту `Тактикум` на `/contacts/`. Real success-flow, staff-order endpoint/upstream, Bitrix admin smoke и Metrika goals закрыты owner evidence. Сайт всё ещё нельзя считать полностью стабилизированным только до финального release commit/no-PII strict sign-off и legacy full-window/source inventory.
