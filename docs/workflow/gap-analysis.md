...
5. Продолжить frontend cleanup: runtime Tailwind/static CSS plan, URL-substring assets и legacy `chat.js` artifact.

## Product Gaps

| ID | Status | Priority | Lane | Area | Gap | Evidence | Impact | Suggested Next Step |
|---|---|---|---|---|---|---|---|---|
| PG-001 | closed | P1 | Full Feature | AI chat / calculator | AI-chat пользовательский сценарий унифицирован для production surfaces | `local/templates/tacticum/js/chat-agent.js`, `header.php`; inline chat удалён из `index.php`, `calculator/index.php`, `price/index.php` | Поведение и ошибки чата теперь чинятся в одном модуле; static `chat.js` остаётся legacy artifact вне production flow | Sprint 03: оформить API contract для `/local/rest/tacticum_chat.php` и решить судьбу legacy `chat.js` |
...
| TG-010 | closed | P2 | Security / Integration | REST method policy | Production prefill flow переведён на POST JSON; legacy GET fallback временно сохранён для совместимости | `tacticum_prefill.php`, `chat-agent.js`, `docs/workflow/chat-offer-contract.md` | Семантика production REST flow выровнена с остальными `/local/rest` endpoints | Позже удалить legacy GET fallback после проверки отсутствия внешних потребителей |
| TG-011 | closed | P2 | Full Feature | Asset loading | Фактическая карта CSS/JS assets зафиксирована, inline-долги и правила описаны | `docs/workflow/asset-layout-audit.md`, `local/templates/tacticum/styles/*.css`, `header.php` | Команда понимает, какие assets production-critical, какие требуют visual regression перед cleanup | Sprint 03: static Tailwind/CSS build plan и component/page-property asset routing |
