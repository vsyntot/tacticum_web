...
Ключевой вывод: `products #21`, `product_blocks #22`, `product_use_cases #23` уже являются рабочим Bitrix source для product page content, но это не финальная semantic storage model для всего сайта. Product FAQ должен перейти в `faq #10`; `clients #8` виден в админке, но ещё не находится в config registry; `feedback/clients` требуют product relation decision; `news.list/services` не должен синтезировать service cards из PHP fallback; generic page sections должны перейти в structured page-content model, а не в narrow catalog iblocks или raw HTML blobs.
