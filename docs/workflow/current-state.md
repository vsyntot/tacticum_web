...
Release evidence refresh 03.06.2026: regenerated production rendered SEO, warning-aware browser action, focused `/price/` and CSS-local visual/action manifests under `/tmp/tacticum-release-closure-2026-06-03/`. Updated `docs/workflow/release-signoff-2026-05-24-post-deploy.draft.json` to those live manifest paths; `npm run release:signoff:summary -- docs/workflow/release-signoff-2026-05-24-post-deploy.draft.json` and draft-check now pass again. `npm run release:public-precheck:prod` also passed on 03.06.2026. Remaining pending gates are manual/external only: `manual-success-flow`, `metrika-goals`, `staff-sale-upstream`.

Manual release gates handoff 03.06.2026: added `tools/release-manual-gates-helper.mjs` / `npm run release:manual-gates:helper`. The helper reads the current sign-off draft when available, lists pending manual gates by default, and prints next actions plus safe evidence skeletons for `manual-success-flow`, `metrika-goals`, `bitrix-admin` and `staff-sale-upstream`. It is read-only: no production requests, no lead creation and no PII collection. If `docs/` is not deployed on production, the helper falls back to standalone skeleton mode instead of failing on missing draft JSON. Production server run from `/home/bitrix/www` on 03.06.2026 confirmed standalone mode for default, focused `staff-sale-upstream` and JSON outputs. This turns the remaining external release work into an owner handoff checklist; after Bitrix admin owner evidence, only Metrika/CRM/upstream/manual flow evidence remain external.

Manual success-flow helper 03.06.2026: added `tools/manual-success-flow-helper.mjs` / `npm run manual:success-flow:helper` to generate controlled payloads, browser snippet, curl templates and safe evidence skeleton for default lead form, modal form, AI chat and prefill without sending requests. The helper keeps lead-creating actions owner-run only and delegates staff-order to `npm run staff:sale:gate-helper`. Production server run from `/home/bitrix/www` on 03.06.2026 confirmed default, browser, curl and release helper outputs. This closes another local handoff/tooling gap for `manual-success-flow`; upstream 502 recovery, Metrika visibility and CRM/upstream IDs remain external evidence.

Metrika goals helper 03.06.2026: added `tools/metrika-goals-helper.mjs` / `npm run metrika:goals:helper` to generate expected goals/events, deployed JS source coverage, browser observer snippet and safe evidence skeleton for `metrika-goals`. Production server run from `/home/bitrix/www` on 03.06.2026 confirmed default, source-check/evidence, browser and release helper outputs; source-check found `missing_events: -`, counter `103471113` in `analytics.js` and `metrika.js`, `reachGoal`, QA browser event and HTTPS Metrika tag. The helper confirms taxonomy and PII rules from deployed JS, but does not access Yandex.Metrika; owner still must confirm goals/events in counter `103471113` or tag manager after controlled flows.
