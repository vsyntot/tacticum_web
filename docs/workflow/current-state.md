...
Price team presets Bitrix model 06.06.2026: быстрые пресеты команды на `/price/` получили доменную runtime-модель `Tacticum\Price` вместо split PHP/JS hardcode. Целевой источник — новые инфоблоки `team_presets` и `team_preset_roles`, где роли пресета связаны с существующим `rates`; transitional fallback сохраняет прежние `mvp`, `discovery`, `support`, `qa-burst` до запуска migration и переключения `price.team_presets_source=bitrix`. `news.list/price` теперь рендерит кнопки и JSON-модель из `TeamPresetService`, карточки ставок экспонируют `data-rate-ids`, JS применяет пресеты по stable rate IDs and uses fallback keywords only in rollout mode. Staff-order endpoint сохраняет прежний `workers_json` / `team_preset` contract, но серверно обогащает task текст label/source/version пресета. Добавлены `price:team-presets:migrate`, `price:team-presets:check` and strict target gate; локальная проверка target rows требует доступного Bitrix DB.
