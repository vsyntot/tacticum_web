...
Specialist order modal для `/price/` находится в Bitrix component template partials `news.list/price/parts/*`; main `script.js` is a short orchestrator, while `price-configurator-*.js` chunks manage filters, prices, segmented level selection, result count, empty state, multi-staff order state, duration presets, team presets, persistent summary, monthly budget estimate and hidden fields through `data-price-*` contracts. Уровни сортируются в компонентном `result_modifier.php` в порядке `Junior -> Middle -> Senior -> Lead`. Frontend отправляет `workers_json`, `duration`, `endDate`, `team_preset`, `monthly_budget_estimate`, а `tacticum_sale_staff.php` сохраняет fallback по legacy `specialist/level/rate`.
