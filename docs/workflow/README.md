...
- `offer-page-taxonomy-presets-decision-2026-06-07.md` — decision baseline: move governed labels/aliases/order/featured presets to Bitrix or equivalent model, keep counts/results runtime-derived and preserve noindex/canonical.
- `offer-page-filter-interaction-challenge-gap-analysis-2026-06-07.md` — source register and local implementation evidence for `/offer/` filter interaction: selected-state visibility, applied chips, `#offer-catalog` fallback, progressive same-URL AJAX, URL/history, accessibility and SEO preservation.
- `offer-page-filter-interaction-roadmap-2026-06-07.md` — phase roadmap/status для закрытия `OFFER-FILTER-*` gaps: state visibility fast fix, progressive enhancement, mobile/a11y/guard evidence and remaining production gates.
- `offer-page-filter-interaction-issue-backlog-2026-06-07.md` — issue backlog/status `OFFER-FILTER-WP-01` - `OFFER-FILTER-WP-04` for `/offer/` filter interaction implementation packages, local evidence and owner gates.
- `public-site-e2e-challenge-gap-analysis-2026-06-07.md` — source register for production public-site E2E challenge: JS/CSS, rendered browser smoke, sitemap/robots, SEO semantics, content quality and page-by-page editorial risks.
- `public-site-e2e-challenge-roadmap-2026-06-07.md` — phase roadmap для закрытия `PUBLIC-E2E-*` gaps: lang/CSS/sitemap hygiene, guard consolidation, Russian-first copy, `/price/` labels, `/offer/` synthetic framing and monitoring.
- `public-site-e2e-challenge-issue-backlog-2026-06-07.md` — issue-ready backlog `PUBLIC-E2E-WP-01` - `PUBLIC-E2E-WP-12` for technical hygiene, SEO/HTTP hardening, editorial owner review and accepted-monitor items.
...
- `plans/2026-06-07-offer-taxonomy-runtime-implementation.md` — implementation plan for WP-04 local offer taxonomy runtime/tooling: fallback/Bitrix service, migration/check/cache scripts and production source-switch gates.
- `plans/2026-06-07-offer-filter-interaction-documentation.md` — docs-only Codex plan for the `/offer/` filter interaction UX/UI challenge package.
- `plans/2026-06-07-offer-filter-interaction-implementation.md` — implementation plan for `/offer/` filter interaction local slice: active state, applied summary, same-URL AJAX/history, accessibility, guards and production smoke gates.
...
- `npm run offer:taxonomy:approval:self-test` / `npm run offer:taxonomy:approval:draft-check` / `npm run offer:taxonomy:approval:check -- <approved.json>` — validates safe `/offer/` taxonomy/preset owner decision artifacts: owners, gates, Russian-first term labels, aliases, featured terms, budget policy, no stored counts, no runtime switch and no iblock apply approval. Self-test is included in `product:content:safety:check`; runtime implementation must reference the approved JSON, while iblock apply and source switch remain separate release gates.
- `npm run offer:taxonomy:implementation-gate:self-test` / `npm run offer:taxonomy:implementation-gate` / `npm run offer:taxonomy:implementation-gate -- --approval=<approved.json>` — static ADR-012 gate that allows the current `CatalogTaxonomy` fast-fix shim, but blocks future `offer_taxonomy_terms`, `offer_filter_presets`, offer-taxonomy source config, migration/cache/finalize scripts or runtime classes unless ADR-012 is accepted and an approved owner JSON is supplied or uniquely discovered in `docs/workflow`.
- `npm run offer:taxonomy:migrate` / `npm run offer:taxonomy:migrate:apply` — Bitrix/PHP dry-run/apply migration for the approved `offer_taxonomy_terms` model. It seeds approved taxonomy terms from owner JSON, or from `tools/offer-taxonomy-approved-model.php` when `/docs` is absent on production, does not approve source switch, and intentionally does not create `offer_filter_presets` while `preset_source=featured_terms`.
- `npm run offer:taxonomy:check` / `npm run offer:taxonomy:check:strict` / `npm run offer:taxonomy:check:json` — Bitrix/PHP runtime/schema checker for offer taxonomy config, terms, aliases, featured options and source mode. Strict mode is for post-apply/source-switch evidence and should not be expected to pass while config is fallback.
- `npm run offer:taxonomy:cache-clear` / `npm run offer:taxonomy:cache-clear:dry-run` / `npm run offer:taxonomy:cache-clear:dry-run:json` — Bitrix/PHP cache-clear helper for offer catalog and taxonomy cache, related managed tags and rendered public cache paths after taxonomy admin edits or source switch.
- `npm run browser:smoke:offer` — Chrome/CDP post-deploy smoke for `/offer/` filter interaction: quick chip apply, active summary, individual remove, form submit, pagination and Back/Forward on desktop/mobile.
