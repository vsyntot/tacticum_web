...
| `npm run offer:taxonomy:migrate`, local 07.06.2026 | Blocked by local Bitrix DB absence: `Mysql connect error [localhost]: (2002) No such file or directory`. Must run as dry-run on target/prod before `--apply`. Production deploy may not include `/docs`; migration falls back to `tools/offer-taxonomy-approved-model.php` when the default approval JSON path is absent. |
