...
| CMP-005 | closed | P2 | Full Feature | `forms.js` used to centralize many flows in one module. | Keep the split forms runtime boundary and preserve payload, consent, CSRF and response model. | Frontend + QA | Contract gate | 05.06.2026: `forms-runtime.js` now owns shared form helpers, product analytics allowlist, returning-lead state and UI error helpers; `forms.js` is a 289-line submit/payload orchestrator; `js:check`, `seo:check`, `bitrix:check` and `git diff --check` pass. |
| CMP-006 | closed | P2 | Full Feature | `chat-agent.js` used to own hero, calculator, price, prefill and lead handoff behaviors. | Keep chat surface adapter and shared transport/state split; preserve `group_id`, prefill and handoff contracts. | Frontend + QA | Integration gate if payload changes | 05.06.2026: split into `chat-runtime.js`, `chat-hero.js`, `chat-calculators.js` and 18-line `chat-agent.js`; `js:check`, `seo:check`, `bitrix:check` and `git diff --check` pass; rendered chat smoke remains post-deploy gate. |
