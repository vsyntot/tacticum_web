...
- Strict proof audit passed on production: `clients 5/5`, `feedback 3/3`, `cases 9/10`, and relevant `PRODUCT` relations link to `products #21`; follow-up audit contract reports per-product aggregate and public-render proof counts without raw copy. Owner decisions are captured in no-raw-copy approval JSON and validated before tagging/public-render apply.
- Proof public-render production closure passed on 06.06.2026: the approved file aligned `cases #181` and `feedback #92` PRODUCT tags, applied 12 `PUBLIC_RENDER_APPROVED=Y` flags, strict proof audit reported all products `public_proof_render_ready=true`, strict product content check reported `proof_source=iblock`, and product HTTP source smoke passed with `proof_source=iblock`.
- Phase 5 boundary guard: `content-storage-aiagents-boundary-check.mjs` keeps `/agents/` as product page and `/aiagents/` as Telegram demo/prototype service route; future demo-agent relation changes remain owner-gated.
...
- Phase 7 wave 2 production evidence: scoped live approval/apply and source HTTP check passed for `/`, `/about/`, `/calculator/` and `/aiagents/`; strict audit reports 20 active sections, 80 active blocks, all wave 2 sections `live` and `orphan_blocks=0`. `/calculator/` required a template correction for `calculator-outcome-cards` to `calculator-chat-outcome`; after correction, `page-content:source:http:wave2:prod` and `seo:check:prod` passed.
...
Future evidence policy:

- No open evidence gate remains in the current content-storage target scope.
- Future proof changes must reuse the no-raw-copy proof approval file, `content-storage-proof-tagging-apply.php`, `content-storage-proof-public-render-apply.php`, strict proof audit and product HTTP `proof_source` smoke before rendering new public proof.
- Future page-content fallback removals should reuse `content:storage:page-content:fallback-retirement-template:wave2` plus `content-storage-page-content-fallback-retirement-check.mjs` before removing any new fallback body.
