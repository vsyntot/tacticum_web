...
Implementation note 07.06.2026: local `CLS-WP-01` implementation normalized product page fallback labels, added `ContentBlockMapper` protection for legacy Bitrix `fits/not_fits/start` names, cleaned page-content seed labels and added source/rendered public hygiene guards. Production baseline before deploy/cache refresh still fails `npm run content:public-hygiene:rendered:prod` with 26 visible issues across product/page-content pages; rerun after deploy is required before using `closed`.
