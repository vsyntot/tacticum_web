...
Статус: challenge source register, not an approval package; `CLS-WP-01` is closed by production rendered hygiene evidence.
...
Implementation note 07.06.2026: `CLS-WP-01` normalized product page fallback labels, added `ContentBlockMapper` protection for legacy Bitrix `fits/not_fits/start` names, cleaned page-content seed labels, added source/rendered public hygiene guards, added exact-phrase runtime normalization for assembled product content/product FAQ/live page-content rows and normalized `tacticum:chat.surface` quick replies. Production rendered hygiene first improved from 26 issues to 4 `/price/` quick-reply issues, then passed after follow-up deploy/cache refresh: `npm run content:public-hygiene:rendered:prod` checked 13 pages with `issues_found=0` at `2026-06-07T06:34:56Z`. The safe JSON evidence is stored in the `content-public-hygiene` gate of `docs/workflow/release-signoff-2026-06-01-product-first.draft.json`.
...
|---|---|---:|---|---|---|---|---|---|
| `CLS-001` | closed | P0 | Product pages / content mapper | Public pages expose internal labels: `Product fit`, `fits`, `not_fits`, `start`, `Use cases`, `Security / procurement`. | Fix public label mapping/source data and add rendered/source guard against service labels in H1-H3/eyebrows. | `CFG-002`, `ARCH-003`, `CONTENT-004`, `UX-010`, `CMP-001` | Backend + Content + QA | Closed by production `npm run content:public-hygiene:rendered:prod` on 07.06.2026: 13 pages, `issues_found=0`; source/rendered guards block recurrence. |
...
| `CLS-006` | in-progress | P1 | `/agents/` vs `/aiagents/` | `/aiagents/` can dilute enterprise Agents positioning and make company look like a Telegram bot shop; `/agents/` comparison card also exposed internal phrasing around the demo route. | Rewrite `/aiagents/` as demo/prototype stand for one Agents scenario; strengthen bridge to `/agents/`. | `UX-004`, `ARCH-010`, `CONTENT-005`, `UI-004`, `PCJMU-005` | PM + SEO + Sales + Content | Copy-only hotfix on `/agents/` card passed production rendered hygiene on 07.06.2026 after product cache clear. Full `/aiagents/` rewrite and any route/canonical decision still require PM/SEO approval; no redirect/canonical change without SEO gate. |
...
| `CLS-012` | closed | P1 | QA / release guard | Current guards do not explicitly scan rendered public HTML for forbidden editorial labels and internal terms. | Add source/rendered content hygiene guard or checklist before content release. | `STACK-004`, `STACK-005`, `ARCH-003`, `REL-002` | QA + Frontend + Backend + Content | Source guard `npm run content:public-hygiene:check`, rendered guard `npm run content:public-hygiene:rendered:prod` and sign-off JSON `npm run content:public-hygiene:rendered:prod:json` exist and passed with production evidence on 07.06.2026. |
...
| `/agents/` | Good enterprise assistant boundary; reported `/aiagents/` demo card now has public-facing copy, but `handoff` and technical/internal terms still repeat heavily. | P1 | `CLS-001`, `CLS-002`, `CLS-004`, `CLS-006` |
...
3. PM/SEO decide remaining route/metadata implications for `/agents/` and `/aiagents/`; copy-only `/agents/` demo card defect is already hotfixed and guarded.
