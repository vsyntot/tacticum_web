...
| CSG-003 | proof audit passed / public rendering implementation-ready | P1 | Product relations | Existing migration adds `PRODUCT` relation to `faq/cases/offer/services/aiagents`, but not `feedback/clients`. | Production migration created `PRODUCT` relation for `feedback #9` and `clients #8`; strict proof-scope audit passed and now reports per-product aggregate and public-render proof counts. Runtime public proof still needs Sales/Content/SEO `PUBLIC_RENDER_APPROVED` evidence before it can replace readiness proof. |
...
| CSG-005 | product tags applied / public rendering implementation-ready | P1 | Cases / Proof | Product proof readiness is not mapped to real cases/evidence; cases are not tagged by product. | Owner-approved `PRODUCT` tags were applied for active `cases`/`feedback` proof items; strict production audit reports `platform=6`, `agents=6`, `dev=6`, `forum=5` aggregate proof items. Runtime/tooling now require a separate `PUBLIC_RENDER_APPROVED=Y` gate and switch `proof_source=iblock` only with at least 3 public-approved proof items per product. Public approval remains pending. |
...
| CSG-007 | wave 1 live / wave 2 shadow-seeded | P1 | Page sections | Many page-level content sections remain PHP partials. | `page_sections #24` and `page_blocks #25` exist; wave 1 seed/live promotion/source switch and targeted Chrome-capable visual/browser smoke passed for four pages. Wave 2 shadow seed passed on production for `/`, `/about/`, `/calculator/` and `/aiagents/`; local runtime guards are ready for deploy, but live promotion/source checks/fallback retirement still need separate owner evidence. |
...
- `tools/content-storage-page-content-seed.php` dry-runs/applies wave 1 and wave 2 page-section rows into `page_sections/page_blocks` in `MIGRATION_STATUS=shadow` only, keeps `FALLBACK_PARTIAL`, prints no raw copy and changes no public runtime. Default remains wave 1; wave 2 is explicit through `--wave=wave_2`.
...
- Production wave 2 shadow seed passed on 06.06.2026 for `/`, `/about/`, `/calculator/` and `/aiagents/`: strict audit reported 20 active sections, 80 active blocks, `orphan_blocks=0`, wave 1 sections still `live` and wave 2 sections still `shadow`; default wave 1 HTTP source check continued to pass with Bitrix markers, and scoped wave 2 fallback HTTP check passed with zero Bitrix markers.
- Local runtime integration now guards wave 2 parts with `tacticum_page_content_render_if_live`, preserves homepage smoke markers and preserves the calculator chat surface through `calculator-chat-outcome`. This is runtime-ready only after deploy and does not approve wave 2 live status, source switch or fallback retirement.
...
Remaining gates are proof/content owner evidence for public rendering and wave 2 page-section live/source evidence. The page-content schema, wave 1 seed, live statuses, public source switch, fallback-retirement checker, owner-approved retirement evidence and post-deploy source/audit/SEO/Chrome smoke evidence now exist; deployed code removed the approved wave 1 fallback bodies. Wave 2 is shadow-seeded on production and runtime-ready locally; scoped live promotion and scoped fallback retirement are now separate owner-gated steps.
...
- Follow-up proof count audit passed after correction: `platform proof_items_total=6`, `agents proof_items_total=6`, `dev proof_items_total=6`, `forum proof_items_total=5`; public product proof rendering remains blocked until `public_render_approved=true` is approved and `PUBLIC_RENDER_APPROVED=Y` is applied.
