...
Post-deploy закрыты `SEO-001`, `SEO-002`, `SEO-003`, `SEO-004`, `SEO-005`, `SEO-006`, `SEO-008`; `SEO-009` принят как UX/navigation decision. Единственный незакрытый production follow-up: `SEO-007`, потому что расширенный `seo:check:prod` обнаружил дубли `<loc>` в динамическом `/offer/sitemap.php`; локальная дедупликация и guard уже добавлены, нужен redeploy и повторный `npm run seo:check:prod`.
...
| Sitemap XML validity | partial | static sitemap ok; dynamic `/offer/sitemap.php` currently has duplicate locs on production until dedupe deploy |
...
| SEO-007 | in_progress | P2 | Fast Fix | Sitemap governance | Static sitemap governance закрыт, но post-deploy расширение проверки выявило duplicate `<loc>` в динамическом `/offer/sitemap.php` | `npm run seo:check:prod` после добавления dynamic guard падает на duplicate locs: `marketpleys-medicinskih-uslug-s-mobilnym-prilozheniem`, `sozdanie-marketpleysa-dlya-medikov-i-medtsentrov`, `sajt-medicinskih-uslug-mvp-dlya-kliniki`; local `offer/sitemap.php` уже дедуплицирует URL, `tools/seo-check.mjs` проверяет dynamic sitemap uniqueness | Без дедупликации поисковики получают повторяющиеся offer URLs в sitemap | Закоммитить/задеплоить dedupe fix и повторить `npm run seo:check:prod`; после pass перевести `SEO-007` в `closed` |
...
- Динамический `/offer/sitemap.php` получает local dedupe fix; production ждёт redeploy.
...
- Gaps `SEO-001` - `SEO-006` и `SEO-008` закрыты production evidence.
- `SEO-007` закрыт локальным кодом и автоматикой, но ждёт redeploy для production evidence.
