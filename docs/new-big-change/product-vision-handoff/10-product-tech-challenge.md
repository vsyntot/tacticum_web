...
| PTC-015 | Components | `product_page.php` теперь bootstrap/helpers, visual blocks вынесены в `product_page_blocks/*.php`; AS IS component/state contract checked; Bitrix component/preview split ещё не решён | Partial split and selector contract lower regression risk, but do not replace TO BE component previews/design specs | Decide whether partials are enough for first release or local components/previews are needed | P2 |
| PTC-016 | Design system | AS IS token contract is checked in `05-design-tokens-as-is.json`, but TO BE source/naming is not approved | Дизайнер уже имеет воспроизводимый baseline, но финальная система не появится без Figma/JSON/Tailwind mapping decision | TO BE token/component/state specification | P1 |
