...
- 01.06.2026: product pages переведены на единый data -> schema -> render flow: `$tacticumProductPage` загружается из `local/php_interface/include/product_data/*.php` and питает HTML, `SoftwareApplication` and `FAQPage` JSON-LD, поэтому видимый static FAQ и structured data синхронизированы.
