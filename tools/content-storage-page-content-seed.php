    ...
    private function waveTwoSections(): array
    {
        return [
            ...
            [
                ...
                'owner_scope' => 'marketing',
                'title' => 'Как мы работаем до запуска',
                'text' => 'Показываем, как команда проходит путь от первичной гипотезы к безопасному первому шагу: сценарий, ответственность, ограничения и формат запуска.',
                'blocks' => [
                    $this->block('hypothesis', 'feature_card', 'Проверяем сценарий', 'Уточняем процесс, пользователей, данные и критерий результата до того, как предлагать пилот или команду.', ['icon' => 'ri-lightbulb-line']),
                    $this->block('responsibility', 'feature_card', 'Разделяем ответственность', 'Фиксируем, что делает клиент, что берет на себя Tacticum и кто поддерживает решение после запуска.', ['icon' => 'ri-eye-line']),
                    $this->block('constraints', 'feature_card', 'Учитываем ограничения', 'Проверяем доступы, безопасность, интеграции, сроки и готовность пользователей к изменению процесса.', ['icon' => 'ri-shape-line']),
                ],
            ],
            [
                'page' => '/about/',
