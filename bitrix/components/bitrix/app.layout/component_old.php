...
)
{
	...
	// get and parse application URL
		...
		$referer = new \Bitrix\Main\Web\Uri($request->getHeader('referer') ?? '');

		$refererPathQuery = null;
		if (!empty($referer->getPath()))
		{
			$requestScheme = $arResult['CURRENT_HOST_SECURE'] ? 'https' : 'http';

 			if (
				$referer->getScheme() === $requestScheme
				&& $referer->getHost() === $request->getHttpHost()
				&& $referer->getPort() === (int)$serverPort
			)
			{
				$refererPathQuery = $referer->getPathQuery();
			}
		}

		if (empty($arParams['PLACEMENT_OPTIONS']) && empty($arParams['~PLACEMENT_OPTIONS']))
		{
			$arParams['PLACEMENT_OPTIONS'] = [];
			$arParams['~PLACEMENT_OPTIONS'] = [];
		}

		if (
			$refererPathQuery !== null
			&& is_array($arParams['PLACEMENT_OPTIONS'])
			&& is_array($arParams['~PLACEMENT_OPTIONS'])
			&& !isset($arParams['PLACEMENT_OPTIONS']['URI'])
			&& !isset($arParams['~PLACEMENT_OPTIONS']['URI'])
		)
		{
			$arParams['PLACEMENT_OPTIONS']['URI'] = $refererPathQuery;
			$arParams['~PLACEMENT_OPTIONS']['URI'] = $refererPathQuery;
		}
