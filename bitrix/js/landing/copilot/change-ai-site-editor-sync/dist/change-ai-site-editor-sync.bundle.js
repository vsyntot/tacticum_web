...
(function (exports, landing_tailwind_runtimesync, landing_copilot_generationObserver, landing_env) {
	'use strict';

	const normalizeTailwindRuntimeEnabled = value => {
	...
	const isTailwindRuntimeEnabled = () => {
		try {
			return normalizeTailwindRuntimeEnabled(landing_env.Env.getInstance().getOptions()?.tailwindRuntimeEnabled);
		} catch {
			return false;
		}
	};
	...
	const reloadCurrentWindow = () => {
		if (landing_tailwind_runtimesync.TailwindRuntimeSync && typeof landing_tailwind_runtimesync.TailwindRuntimeSync.reloadWindow === 'function') {
	...
	class ChangeAiSiteEditorSync {
		...
		static preloadTailwindRuntime(event, observer) {
			...
			tailwindRuntimeSync.preloadRuntimeForWindow(window, {
		...
		static sync(event, observer) {
			...
			const syncPromise = this.#syncFonts(fonts).then(() => {
				return Array.isArray(actions) && actions.length > 0 ? this.#syncActions(actions, landingId) : this.#reloadHistory();
			});
		...
		static logStart(event, observer) {}
		static logTargetBlocksResolved(event, observer) {}
		static #isSupportedCommand(command) {
			return [this.COMMAND_START, this.COMMAND_TARGET_BLOCKS_RESOLVED, this.COMMAND_TAILWIND_PRELOAD, this.COMMAND_FINISH].includes(command);
		}
		static #isMatchingEvent(event, observer) {
			...
			if (generationId > 0 && event?.params?.generationId !== undefined && event.params.generationId !== generationId) {
				return false;
			}
			return true;
		}
		static #resolveEventLandingId(event) {
			return Math.max(0, parseInt(event?.params?.landingId, 10) || 0);
		}
		static #normalizeEditorEventPayload(event, observer) {
			const params = event?.params?.params && typeof event.params.params === 'object' ? event.params.params : {};
			return {
		...
		static #normalizeEditorActions(actions) {
		...
		static #normalizeEditorHtmlBlocks(htmlBlocks) {
			if (!Array.isArray(htmlBlocks)) {
				return [];
			}
			return htmlBlocks.filter(htmlBlock => htmlBlock && typeof htmlBlock === 'object').map(htmlBlock => ({
		...
		static #normalizeStringList(items) {
			if (!Array.isArray(items)) {
				return [];
			}
			return items.map(item => String(item || '').trim()).filter(item => item.length > 0);
		}
		static #normalizeIdList(items) {
			if (!Array.isArray(items)) {
				return [];
			}
			return items.map(item => Math.max(0, parseInt(item, 10) || 0)).filter(item => item > 0);
		}
		static #normalizeSkippedReasons(reasons) {
		...
		static resolveCurrentLandingId() {
			try {
		...
		static #isEditorContextAvailable() {
			return !!BX?.Landing?.Main?.getInstance && !!BX?.Landing?.PageObject?.getBlocks && !!BX?.Landing?.Backend?.getInstance;
		}
		static #resolveTailwindRuntimeHelpersBasePath() {
		...
		static #syncActions(actions, landingId) {
		...
		static #syncActionsWithoutTailwind(actions, landingId) {
			return actions.reduce((promise, action) => {
				return promise.then(() => {
					return this.#applyAction(action, landingId).catch(error => {
		...
		static #syncActionsWithTailwind(actions, landingId) {
			...
			if (typeof landing_tailwind_runtimesync.TailwindRuntimeBatchSync !== 'function') {
			...
			return batchSync.prepare().then(() => {
				return actions.reduce((promise, action) => {
					return promise.then(() => {
						...
						return this.#applyAction(action, landingId).then(() => {
		...
		static #normalizeTailwindOperation(action) {
		...
		static #normalizeTailwindOperations(actions) {
			if (!Array.isArray(actions)) {
				return [];
			}
			return actions.map(action => this.#normalizeTailwindOperation(action)).filter(Boolean);
		}
		static #isTailwindSyncRequired(actions) {
			return landing_tailwind_runtimesync.getPendingBlockIds(this.#normalizeTailwindOperations(actions)).size > 0;
		}
		static #getTailwindRuntimeSync() {
			return landing_tailwind_runtimesync.TailwindRuntimeSync || null;
		}
		static #syncFonts(fonts) {
			if (!Array.isArray(fonts) || fonts.length <= 0 || !document?.head) {
				return Promise.resolve();
			}
			return Promise.all(fonts.map(font => this.#appendFont(font))).then(() => {});
		}
		static #appendFont(font) {
		...
		static #hasFontLink(className) {
			return this.#hasHeadElement('data-font', className);
		}
		static #hasFontStyle(className) {
			return this.#hasHeadElement('data-id', className);
		}
		static #hasHeadElement(attribute, value) {
			if (!document?.head?.querySelectorAll) {
				return false;
			}
			return [...document.head.querySelectorAll(`[${attribute}]`)].some(element => element.getAttribute(attribute) === value);
		}
		static #buildFontHref(name) {
		...
		static #applyAction(action, landingId) {
			const normalizedAction = this.#normalizeAction(action);
			if (!normalizedAction) {
				return Promise.resolve();
			}
		...
		static #normalizeAction(action) {
			...
			const placement = hasResolvedPlacement ? resolvedPlacement : requestedPlacement;
			return {
				type,
				status,
				blockId,
		...
		static #addBlock(blockId, landingId, placement) {
		...
		static #insertBlock(response, landingId, placement) {
			const main = BX.Landing.Main.getInstance();
			const previousState = {
			...
			return main.addBlock(response, true).then(result => {
		...
		static #restoreInsertState(main, previousState) {
		...
		static #resolveInsertContext(landingId, placement) {
			const normalizedPlacement = this.#normalizePlacement(placement);
			const targetBlock = normalizedPlacement.blockId > 0 ? this.#getBlockById(normalizedPlacement.blockId) : null;
			if (targetBlock) {
				return {
					block: targetBlock,
		...
		static #moveBlock(blockId, landingId, placement) {
			const normalizedPlacement = this.#normalizePlacement(placement);
		...
		static #resolveLandingArea(landingId) {
			const selectors = [`.landing-area[data-landing="${landingId}"]`, `[data-landing="${landingId}"]`, '.landing-area'];
		...
		static #resolveMoveContext(landingId, sourceNode, targetNode, placement) {
		...
		static #isMoveNoop(sourceNode, placement, referenceNode) {
		...
		static #normalizeLogContext(context = {}) {
			const placement = context?.placement && typeof context.placement === 'object' ? this.#normalizePlacement(context.placement) : undefined;
			return {
		...
		static #normalizePlacement(placement) {
			const normalized = placement && typeof placement === 'object' ? placement : {};
			const mode = ['append', 'prepend', 'after', 'before'].includes(normalized.mode) ? normalized.mode : 'append';
			return {
				mode,
	...
	landing_copilot_generationObserver.GenerationObserver.registerLandingIdResolver(() => ChangeAiSiteEditorSync.resolveCurrentLandingId());
	landing_copilot_generationObserver.GenerationObserver.registerPullHandler(ChangeAiSiteEditorSync);
	const __testHooks = {
		setReloadEditorWindowHandler(handler) {
			reloadEditorWindowHandler = typeof handler === 'function' ? handler : reloadCurrentWindow;
		},
		resetReloadEditorWindowHandler() {
			reloadEditorWindowHandler = reloadCurrentWindow;
		}
	};

	exports.ChangeAiSiteEditorSync = ChangeAiSiteEditorSync;
	exports.__testHooks = __testHooks;

})(this.BX.Landing.Copilot = this.BX.Landing.Copilot || {}, BX.Landing, BX.Landing.Copilot, BX.Landing);
//# sourceMappingURL=change-ai-site-editor-sync.bundle.js.map
