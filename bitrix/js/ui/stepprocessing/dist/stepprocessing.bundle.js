...
(function (exports, main_core, main_core_events, ui_designTokens, ui_progressbar, main_popup, ui_alerts, ui_buttons) {
	'use strict';
	...
	class BaseField {
		...
		getContainer() {
			if (!this.container) {
				this.container = main_core.Tag.render`<div class="${DialogStyle.ProcessOptionContainer} ${this.className}"></div>`;
				if (this.title) {
					this.container.appendChild(main_core.Tag.render`<div class="${DialogStyle.ProcessOptionsTitle}"></div>`).appendChild(main_core.Tag.render`<label for="${this.id}_inp">${this.title}</label>`);
				}
				this.container.appendChild(main_core.Tag.render`<div class="${DialogStyle.ProcessOptionsInput}"></div>`).appendChild(this.render());
				if (this.obligatory) {
					const alertId = this.id + '_alert';
					this.container.appendChild(main_core.Tag.render`<div id="${alertId}" class="${DialogStyle.ProcessOptionsObligatory}" style="display:none"></div>`).appendChild(main_core.Tag.render`<span class="ui-alert-message">${this.emptyMessage}</span>`);
				}
			}
		...
		showWarning(message) {
			...
			if (optionElement) {
				...
				this.container.appendChild(main_core.Tag.render`<div id="${alertId}" class="${DialogStyle.ProcessOptionsObligatory}"></div>`).appendChild(main_core.Tag.render`<span class="ui-alert-message">${messageText}</span>`);
			}
			return this;
		}
		hideWarning() {
	...
	class TextField extends BaseField {
		type = 'text';
		className = DialogStyle.ProcessOptionText;
		rows = 10;
		cols = 50;
		constructor(options) {
			super(options);
		...
		render() {
			if (!this.field) {
				this.field = main_core.Tag.render`<textarea id="${this.id}" name="${this.name}" cols="${this.cols}" rows="${this.rows}"></textarea>`;
			}
			if (this.value) {
				this.field.value = this.value;
			}
	...
	class FileField extends BaseField {
		type = 'file';
		className = DialogStyle.ProcessOptionFile;
		constructor(options) {
		...
		render() {
			if (!this.field) {
				this.field = main_core.Tag.render`<input type="file" id="${this.id}" name="${this.name}">`;
			}
			return this.field;
		}
		lock(flag = true) {
	...
	class CheckboxField extends BaseField {
		...
		list = [];
		multiple = false;
		className = DialogStyle.ProcessOptionCheckbox;
		constructor(options) {
			super(options);
			if ('list' in options) {
		...
		render() {
			if (!this.field) {
				this.field = main_core.Tag.render`<div id="${this.id}"></div>`;
			}
			if (this.multiple) {
				Object.keys(this.list).forEach(itemId => {
					if (this.value.indexOf(itemId) !== -1) {
						this.field.appendChild(main_core.Tag.render`<label><input type="checkbox" name="${this.name}[]" value="${itemId}" checked>${this.list[itemId]}</label>`);
					} else {
						this.field.appendChild(main_core.Tag.render`<label><input type="checkbox" name="${this.name}[]" value="${itemId}">${this.list[itemId]}</label>`);
					}
				});
			} else {
				if (main_core.Type.isBoolean(this.value)) {
					if (this.value) {
						this.field.appendChild(main_core.Tag.render`<input type="checkbox" id="${this.id}_inp" name="${this.name}" value="Y" checked>`);
					} else {
						this.field.appendChild(main_core.Tag.render`<input type="checkbox" id="${this.id}_inp" name="${this.name}" value="Y">`);
					}
				} else {
					if (this.value !== '') {
						this.field.appendChild(main_core.Tag.render`<input type="checkbox" id="${this.id}_inp" name="${this.name}" value="${this.value}" checked>`);
					} else {
						this.field.appendChild(main_core.Tag.render`<input type="checkbox" id="${this.id}_inp" name="${this.name}" value="${this.value}>"`);
					}
				}
			}
			return this.field;
	...
	class SelectField extends BaseField {
		...
		multiple = false;
		list = [];
		className = DialogStyle.ProcessOptionSelect;
		constructor(options) {
			super(options);
		...
		render() {
			if (!this.field) {
				this.field = main_core.Tag.render`<select id="${this.id}" name="${this.name}"></select>`;
			}
			if (this.multiple) {
			...
			Object.keys(this.list).forEach(itemId => {
				...
				let option = this.field.appendChild(main_core.Tag.render`<option value="${itemId}">${this.list[itemId]}</option>`);
				if (selected) {
	...
	class RadioField extends BaseField {
		type = 'radio';
		list = [];
		className = DialogStyle.ProcessOptionMultiple;
		constructor(options) {
			super(options);
			if ('list' in options) {
				this.list = options.list;
		...
		render() {
			...
			Object.keys(this.list).forEach(itemId => {
				if (itemId === this.value) {
					this.field.appendChild(main_core.Tag.render`<label><input type="radio" name="${this.name}" value="${itemId}" checked>${this.list[itemId]}</label>`);
				} else {
					this.field.appendChild(main_core.Tag.render`<label><input type="radio" name="${this.name}" value="${itemId}">${this.list[itemId]}</label>`);
				}
			});
			return this.field;
		}
	...
	class Dialog {
		...
		_prepareDialogContent() {
			this.summaryBlock = main_core.Tag.render`<div class="${DialogStyle.ProcessSummary}">${this.getMessage('summary')}</div>`;
			...
			if (this.progressBar) {
				this.progressBarBlock = main_core.Tag.render`<div class="${DialogStyle.ProcessProgressbar}" style="display:none"></div>`;
				this.progressBarBlock.appendChild(this.progressBar.getContainer());
			}
			if (!this.optionsFieldsBlock) {
				this.optionsFieldsBlock = main_core.Tag.render`<div class="${DialogStyle.ProcessOptions}" style="display:none"></div>`;
			} else {
			...
			let dialogContent = main_core.Tag.render`<div class="${DialogStyle.ProcessPopup}"></div>`;
	...
	class Process {
		...
		queue = [];
		currentStep = -1;
		state = ProcessState.intermediate;
...
})(this.BX.UI.StepProcessing = this.BX.UI.StepProcessing || {}, BX, BX.Event, BX, BX.UI, BX.Main, BX.UI, BX.UI);
