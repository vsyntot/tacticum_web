...
(function (exports,ui_vue3) {
	...
	/*!
	  ...
	  * @license MIT
	  *
	  * @source: https://unpkg.com/vue-router@5.0.2/dist/vue-router.esm-browser.js
	  */
	...
	function registerGuard(activeRecordRef, name, guard) {
	  ...
	  ui_vue3.onUnmounted(removeFromList);
	  ui_vue3.onDeactivated(removeFromList);
	  ui_vue3.onActivated(() => {
	...
	function onBeforeRouteLeave(leaveGuard) {
	  if (!ui_vue3.getCurrentInstance()) {
	    warn("getCurrentInstance() returned null. onBeforeRouteLeave() must be called at the top of a setup function");
	    return;
	  }
	  registerGuard(ui_vue3.inject(matchedRouteKey, {}), "leaveGuards", leaveGuard);
	}
	/**
	...
	function onBeforeRouteUpdate(updateGuard) {
	  ...
	  registerGuard(ui_vue3.inject(matchedRouteKey, {}), "updateGuards", updateGuard);
	}
	...
	function useLink(props) {
	  const router = ui_vue3.inject(routerKey);
	  const currentRoute = ui_vue3.inject(routeLocationKey);
	  let hasPrevious = false;
	  let previousTo = null;
	  const route = ui_vue3.computed(() => {
	    const to = ui_vue3.unref(props.to);
	  ...
	  const activeRecordIndex = ui_vue3.computed(() => {
	    const {
	      matched
	    } = route.value;
	    const {
	  ...
	  const isActive = ui_vue3.computed(() => activeRecordIndex.value > -1 && includesParams(currentRoute.params, route.value.params));
	  const isExactActive = ui_vue3.computed(() => activeRecordIndex.value > -1 && activeRecordIndex.value === currentRoute.matched.length - 1 && isSameRouteLocationParams(currentRoute.params, route.value.params));
	  function navigate(e = {}) {
	    if (guardEvent(e)) {
	      const p = router[ui_vue3.unref(props.replace) ? "replace" : "push"](ui_vue3.unref(props.to)).catch(noop);
	  ...
	  if (isBrowser) {
	    const instance = ui_vue3.getCurrentInstance();
	    if (instance) {
	      ...
	      ui_vue3.watchEffect(() => {
	        ...
	        linkContextDevtools.error = isRouteLocation(ui_vue3.unref(props.to)) ? null : "Invalid \"to\" value";
	      }, {
	        flush: "post"
	      });
	    }
	  ...
	  return {
	    route,
	    href: ui_vue3.computed(() => route.value.href),
	    isActive,
	    isExactActive,
	    navigate
	  };
	...
	const RouterLinkImpl = /* @__PURE__ */ui_vue3.defineComponent({
	  ...
	  setup(props, {
	    ...
	    const link = ui_vue3.reactive(useLink(props));
	    const {
	      options
	    } = ui_vue3.inject(routerKey);
	    const elClass = ui_vue3.computed(() => ({
	      [getLinkClass(props.activeClass, options.linkActiveClass, "router-link-active")]: link.isActive,
	      [getLinkClass(props.exactActiveClass, options.linkExactActiveClass, "router-link-exact-active")]: link.isExactActive
	    }));
	    return () => {
	      const children = slots.default && preferSingleVNode(slots.default(link));
	      return props.custom ? children : ui_vue3.h("a", {
	...
	const RouterViewImpl = /* @__PURE__ */ui_vue3.defineComponent({
	  name: "RouterView",
	  inheritAttrs: false,
	  props: {
	    name: {
	  ...
	  setup(props, {
	    ...
	    const injectedDepth = ui_vue3.inject(viewDepthKey, 0);
	    ...
	    const viewRef = ui_vue3.ref();
	    ui_vue3.watch(() => [viewRef.value, matchedRouteRef.value, props.name], ([instance, to, name], [oldInstance, from, oldName]) => {
	      if (to) {
	    ...
	    return () => {
	      ...
	      const component = ui_vue3.h(ViewComponent, assign({}, routeProps, attrs, {
	        onVnodeUnmounted,
	        ref: viewRef
	      }));
  ...
  <${comp}>\n    <component :is="Component" />\n  </${comp}>\n</router-view>`);
	...
	function addDevtools(app, router, matcher) {
	  ...
	  setupDevtoolsPlugin({
	    ...
	    ui_vue3.watch(router.currentRoute, () => {
	      refreshRoutesView();
	...
	function createRouter(options) {
	  ...
	  const currentRoute = ui_vue3.shallowRef(START_LOCATION_NORMALIZED);
	  ...
	  function removeRoute(name) {
	    ...
	    if (recordMatcher) matcher.removeRoute(recordMatcher);else warn(`Cannot remove non-existent route "${String(name)}"`);
	  }
	  function getRoutes() {
	  ...
	  function resolve(rawLocation, currentLocation) {
	    ...
	    if (typeof rawLocation === "string") {
	      ...
	      if (href.startsWith("//")) warn(`Location "${rawLocation}" resolved to "${href}". A resolved location cannot start with multiple slashes.`);else if (!matchedRoute.matched.length) warn(`No match found for location with path "${rawLocation}"`);
	    ...
	    if (href.startsWith("//")) warn(`Location "${rawLocation}" resolved to "${href}". A resolved location cannot start with multiple slashes.`);else if (!matchedRoute.matched.length) warn(`No match found for location with path "${rawLocation.path != null ? rawLocation.path : rawLocation}"`);
	    return assign({
	      fullPath,
	      hash,
	  ...
	  function handleScroll(to, from, isPush, isFirstNavigation) {
	    ...
	    return ui_vue3.nextTick().then(() => scrollBehavior(to, from, scrollPosition)).then(position => position && scrollToPosition(position)).catch(err => triggerError(err, to, from));
	  }
	  ...
	  const router = {
	    ...
	    install(app) {
	      ...
	      Object.defineProperty(app.config.globalProperties, "$route", {
	        enumerable: true,
	        get: () => ui_vue3.unref(currentRoute)
	      });
	      ...
	      app.provide(routeLocationKey, ui_vue3.shallowReactive(reactiveRoute));
	 ...
	 */
	function useRouter() {
	  return ui_vue3.inject(routerKey);
	}
	/**
	...
	function useRoute(_name) {
	  return ui_vue3.inject(routeLocationKey);
	}
	// origin-end
...
}((this.BX.Vue3.VueRouter = this.BX.Vue3.VueRouter || {}),BX.Vue3));
//# sourceMappingURL=vue-router.bundle.js.map
