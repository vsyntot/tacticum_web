...
?>
					<td title="<?=htmlspecialcharsbx($tab["title"] ?? '')?>" id="tab_cont_<?=$tab["id"]?>" class="bx-tab-container<?=($bSelected? "-selected":"")?>" onclick="<?php if($callback <> ''):?><?= $callback ?>('<?=$tab["id"]?>');<?php endif?>bxForm_<?=$arParams["FORM_ID"]?>.SelectTab('<?=$tab["id"]?>');" onmouseover="if(window.bxForm_<?=$arParams["FORM_ID"]?>){bxForm_<?=$arParams["FORM_ID"]?>.HoverTab('<?=$tab["id"]?>', true);}" onmouseout="if(window.bxForm_<?=$arParams["FORM_ID"]?>){bxForm_<?=$arParams["FORM_ID"]?>.HoverTab('<?=$tab["id"]?>', false);}">
						<table cellspacing="0">
							<tr>
...
endforeach;
?>
					<td width="100%"<?php if($USER->IsAuthorized() && $arParams["SHOW_SETTINGS"] == true):?> ondblclick="bxForm_<?=$arParams["FORM_ID"]?>.ShowSettings()"<?php endif?> style="white-space:nowrap; text-align:right">
<?php if(count($arResult["TABS"]) > 1 && $arParams["CAN_EXPAND_TABS"] == true):?>
<a href="javascript:void(0)" onclick="bxForm_<?=$arParams["FORM_ID"]?>.ToggleTabs();" title="<?= GetMessage("interface_form_show_all")?>" id="bxForm_<?=$arParams["FORM_ID"]?>_expand_link" class="bx-context-button bx-down"><span></span></a>
<?php endif?>
<?php if($arParams["SHOW_SETTINGS"] || !empty($arThemes)):?>
<a href="javascript:void(0)" onclick="bxForm_<?=$arParams["FORM_ID"]?>.menu.ShowMenu(this, bxForm_<?=$arParams["FORM_ID"]?>.settingsMenu);" title="<?= GetMessage("interface_form_settings")?>" class="bx-context-button bx-form-menu"><span></span></a>
<?php endif;?>
					</td>
				</tr>
			</table>
...
<?php
	...
	if(!isset($field["colspan"]) || $field["colspan"] <> true):
		if(isset($field["required"]) && $field["required"])
			$bWasRequired = true;
?>
		<td class="bx-field-name<?php if(!isset($field["type"]) || $field["type"] <> 'label') echo' bx-padding'?>"<?php if(isset($field["title"]) && $field["title"] <> '') echo ' title="'.htmlspecialcharsEx($field["title"]).'"'?>><?=(isset($field["required"]) && $field["required"]? '<span class="required">*</span>':'')?><?php if(isset($field["name"]) && $field["name"] <> ''):?><?= htmlspecialcharsEx($field["name"]) ?>:<?php endif?></td>
	<?php
	endif
?>
...
<?php if(!isset($arParams["~BUTTONS"]["standard_buttons"]) || $arParams["~BUTTONS"]["standard_buttons"] !== false):?>
	<?php if($arParams["BUTTONS"]["back_url"] <> ''):?>
	<input type="submit" name="save" value="<?= GetMessage("interface_form_save")?>" title="<?= GetMessage("interface_form_save_title")?>" />
	<?php endif?>
	<input type="submit" name="apply" value="<?= GetMessage("interface_form_apply")?>" title="<?= GetMessage("interface_form_apply_title")?>" />
	<?php if($arParams["BUTTONS"]["back_url"] <> ''):?>
	<input type="button" value="<?= GetMessage("interface_form_cancel")?>" name="cancel" onclick="window.location='<?=htmlspecialcharsbx(CUtil::addslashes($arParams["~BUTTONS"]["back_url"]))?>'" title="<?= GetMessage("interface_form_cancel_title")?>" />
	<?php endif?>
<?php endif?>
...
<?php if($USER->IsAuthorized() && $arParams["SHOW_SETTINGS"] == true):?>
<div style="display:none">

<div id="form_settings_<?=$arParams["FORM_ID"]?>">
<table width="100%">
	<tr class="section">
		<td><?= GetMessage("interface_form_tabs")?></td>
	</tr>
	<tr>
		<td align="center">
			<table>
				<tr>
					...
					<td style="background-image:none">
						<div style="margin-bottom:5px"><input type="button" name="tab_up_btn" value="<?= GetMessage("intarface_form_up")?>" title="<?= GetMessage("intarface_form_up_title")?>" style="width:80px;" onclick="bxForm_<?=$arParams["FORM_ID"]?>.TabMoveUp()"></div>
						<div style="margin-bottom:5px"><input type="button" name="tab_down_btn" value="<?= GetMessage("intarface_form_up_down")?>" title="<?= GetMessage("intarface_form_down_title")?>" style="width:80px;" onclick="bxForm_<?=$arParams["FORM_ID"]?>.TabMoveDown()"></div>
						<div style="margin-bottom:5px"><input type="button" name="tab_add_btn" value="<?= GetMessage("intarface_form_add")?>" title="<?= GetMessage("intarface_form_add_title")?>" style="width:80px;" onclick="bxForm_<?=$arParams["FORM_ID"]?>.TabAdd()"></div>
						<div style="margin-bottom:5px"><input type="button" name="tab_edit_btn" value="<?= GetMessage("intarface_form_edit")?>" title="<?= GetMessage("intarface_form_edit_title")?>" style="width:80px;" onclick="bxForm_<?=$arParams["FORM_ID"]?>.TabEdit()"></div>
						<div style="margin-bottom:5px"><input type="button" name="tab_del_btn" value="<?= GetMessage("intarface_form_del")?>" title="<?= GetMessage("intarface_form_del_title")?>" style="width:80px;" onclick="bxForm_<?=$arParams["FORM_ID"]?>.TabDelete()"></div>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr class="section">
		<td><?= GetMessage("intarface_form_fields")?></td>
	</tr>
	<tr>
		<td align="center">
			<table>
				<tr>
					<td style="background-image:none" nowrap>
						<div style="margin-bottom:5px"><?= GetMessage("intarface_form_fields_available")?></div>
					...
					<td style="background-image:none">
						<div style="margin-bottom:5px"><input type="button" name="add_btn" value="&gt;" title="<?= GetMessage("intarface_form_add_field")?>" style="width:30px;" disabled onclick="bxForm_<?=$arParams["FORM_ID"]?>.FieldsAdd()"></div>
						<div style="margin-bottom:5px"><input type="button" name="del_btn" value="&lt;" title="<?= GetMessage("intarface_form_del_field")?>" style="width:30px;" disabled onclick="bxForm_<?=$arParams["FORM_ID"]?>.FieldsDelete()"></div>
					</td>
					<td style="background-image:none" nowrap>
						<div style="margin-bottom:5px"><?= GetMessage("intarface_form_fields_on_tab")?></div>
					...
					<td style="background-image:none">
						<div style="margin-bottom:5px"><input type="button" name="up_btn" value="<?= GetMessage("intarface_form_up")?>" title="<?= GetMessage("intarface_form_up_title")?>" style="width:80px;" disabled onclick="bxForm_<?=$arParams["FORM_ID"]?>.FieldsMoveUp()"></div>
						<div style="margin-bottom:5px"><input type="button" name="down_btn" value="<?= GetMessage("intarface_form_up_down")?>" title="<?= GetMessage("intarface_form_down_title")?>" style="width:80px;" disabled onclick="bxForm_<?=$arParams["FORM_ID"]?>.FieldsMoveDown()"></div>
						<div style="margin-bottom:5px"><input type="button" name="field_add_btn" value="<?= GetMessage("intarface_form_add")?>" title="<?= GetMessage("intarface_form_add_sect")?>" style="width:80px;" onclick="bxForm_<?=$arParams["FORM_ID"]?>.FieldAdd()"></div>
						<div style="margin-bottom:5px"><input type="button" name="field_edit_btn" value="<?= GetMessage("intarface_form_edit")?>" title="<?= GetMessage("intarface_form_edit_field")?>" style="width:80px;" onclick="bxForm_<?=$arParams["FORM_ID"]?>.FieldEdit()"></div>
					</td>
				</tr>
			</table>
		</td>
...
<?php if($arResult["IS_ADMIN"]):?>
	<tr class="section">
		<td><?= GetMessage("interface_form_common")?></td>
	</tr>
	<tr>
		<td><input type="checkbox" name="set_default_settings" id="set_default_settings_<?=$arParams["FORM_ID"]?>" onclick="BX('delete_users_settings_<?=$arParams["FORM_ID"]?>').disabled = !this.checked;"><label for="set_default_settings_<?=$arParams["FORM_ID"]?>"><?= GetMessage("interface_form_common_set")?></label></td>
	</tr>
	<tr>
		<td><input type="checkbox" name="delete_users_settings" id="delete_users_settings_<?=$arParams["FORM_ID"]?>" disabled><label for="delete_users_settings_<?=$arParams["FORM_ID"]?>"><?= GetMessage("interface_form_common_del")?></label></td>
	</tr>
<?php endif;?>
</table>
</div>
...
<?php if($arParams["SHOW_SETTINGS"] == true):?>
...
<?php if(isset($arResult["OPTIONS"]["expand_tabs"]) && $arResult["OPTIONS"]["expand_tabs"] == "Y"):?>
...
<?php if($bWasRequired):?>
<div class="bx-form-notes"><span class="required">*</span><?= GetMessage("interface_form_required")?></div>
<?php endif?>
