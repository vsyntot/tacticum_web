...
else
{
	...
	<div class="bx-gadgetsadm-list-table-layout">
		...
		<div class="bx-gadgets-content" style="padding: 25px;">
			...
			<?php
			$ht = new Bitrix\Main\Web\HttpClient(["socketTimeout" => 30]);
			$arF = [
			...
			if ($res = $ht->post($domain . "/key_update.php", $arF))
			{
					...
					try
					{
						...
						if ($res["result"] == "ok")
						{
							...
							<div style="background-color: #f2f6f7; padding: 10px 20px 15px 20px; font-size: 15px; border-radius: 4px;">
								<div style="font-weight: bold; line-height: 2em;">
									<span style="color: #798284;"><?= GetMessage("BUY_SUP_PARTNER") ?></span>&nbsp;
									<?php
									if ($res["link"] <> '')
									{
										?>
									...
									if ($res["phone"] <> '')
									{
										...
										<span style="color: #798284;"><?= GetMessage("BUY_SUP_PHONE") ?></span>&nbsp;<?= $res["phone"] ?>
										</div><?php
									}
									if ($res["email"] <> '')
									{
										...
										<span style="color: #798284;"><?= GetMessage("BUY_SUP_EMAIL") ?></span>&nbsp;<a href="mailto:<?= $res["email"] ?>" style="color: #000; text-decoration: none;"><?= $res["email"] ?></a>
										</div><?php
									}
									?>
								</div>
				...
				<div style="position: relative; padding: 25px; border: 1px solid #859f4a; border-radius: 4px; margin-top: 40px;">
					<div style="font-size: 18px; line-height: 28px; position: absolute; top: -14px; left: 14px; display: block; height: 28px; margin: 0; padding: 0 10px; vertical-align: middle; color: #859f4a; background: #fff; font-weight: bold;"><?= GetMessage("BUY_SUP_TOBUY") ?></div>
					<?php
					$ht = new Bitrix\Main\Web\HttpClient(["socketTimeout" => 30]);
					$arF = [
						"license_key" => $lkeySign,
						...
						if ($ht->getStatus() == "200")
						{
							...
							if ($res["result"] == "ok")
							{
								...
								else
								{
									...
									echo GetMessage("BUY_SUP_AMOUNT", ["#AMOUNT#" => $res["price"]]);
								}
								$buyUrl = $res["toBasket"];
								?></div><?php
							}
					...
					<div id="ok" style="display: none; color: #859f4a; font-weight: bold;">
						<br/><br/><?= GetMessage("BUY_SUP_CONTACT_OK2") ?></div>
					<div id="req">
						<br/><br/>
						<table style="border-bottom: 1px solid #dbdbda;">
							<tr>
								...
								<td style="padding: 5px 0 5px 15px;">
									<input type="text" name="name" value="<?= htmlspecialcharsbx($USER->GetFullName()) ?>" id="name">
								</td>
							...
							<tr>
								...
								<td style="padding: 5px 0 5px 15px;">
									<input type="text" name="phone" value="" id="phone"></td>
							</tr>
							<tr>
								<td nowrap valign="middle"><?= GetMessage("BUY_SUP_YEMAIL") ?></td>
								<td style="padding: 5px 0 5px 15px;">
									<input type="text" name="email" value="<?= htmlspecialcharsbx($USER->GetEmail()) ?>" id="email">
								</td>
							</tr>
							<tr>
								<td colspan="3">&nbsp;</td>
