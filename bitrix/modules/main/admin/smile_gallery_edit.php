...
if ($_SERVER['REQUEST_METHOD'] == "POST" && (!empty($_POST['save']) || !empty($_POST['apply'])))
{
	...
	if (empty($arError))
	{
		...
		else
		{
			LocalRedirect(
				(!empty($_POST['save']) ?
					"smile_gallery.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false) :
					"smile_gallery_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&".GetFilterParams("filter_", false)));
		}
	}
...
$aMenu = array(
	array(
		...
		"LINK" => "/bitrix/admin/smile_gallery.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false),
		"ICON" => "btn_list",
	)
);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/bitrix/admin/smile_gallery_edit.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false),
		"ICON" => "btn_new",
	);
	if($arSmileSet["STRING_ID"] != 'bitrix')
	{
		$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("SMILE_BTN_DELETE_CONFIRM")."')) window.location='/bitrix/admin/smile_gallery.php?action=delete&ID[]=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."#tb';",
			"ICON" => "btn_delete",
		);
	}
}
...
$tabControl->Buttons(array(
	"back_url" => "/bitrix/admin/smile_gallery.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)));
?>
</form>
<?php
$tabControl->End();
